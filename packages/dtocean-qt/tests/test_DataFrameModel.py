...
@pytest.mark.parametrize(
    "copy, operator", [(True, numpy.not_equal), (False, numpy.equal)]
)
...
def test_columnCount():
    ...
    model = DataFrameModel(
        pandas.DataFrame(
            numpy.arange(100).reshape(1, 100), columns=numpy.arange(100)
        )
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                1.11111,
                numpy.float16,
                DataFrameModel._float_precisions[str("float16")],
            ),
            (
                1.11111111,
                numpy.float32,
                DataFrameModel._float_precisions[str("float32")],
            ),
            (
                1.1111111111111111,
                numpy.float64,
                DataFrameModel._float_precisions[str("float64")],
            ),
        ],
    )
