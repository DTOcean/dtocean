...
class Loader:
    ...
    def get_data_value(self, pool, simulation, data_identity):
        ...
        merged_state = self.create_merged_state(simulation)
        data_value = self._store.get_data_value(
            pool, merged_state, data_identity
        )

        return data_value

    def input_available(self, pool, simulation, interface, check_id):
        ...
        result = False
        if check_id in active_inputs:
            result = True

        return result

    def can_load(self, pool, simulation, interface):
    ...
    def load_interface(self, pool, simulation, interface, skip_vars=None):
        input_declaration, optional_inputs = interface.get_inputs()
        ...
        for putvar in active_inputs:
            ...
            if data_value is None and putvar not in optional_inputs:
                errStr = (
                    "Input {} is required by interface {} but is"
                    "not satisfied."
                ).format(putvar, interface.get_name())
    ...
    def add_datastate(
        ...
        if identifiers is None or values is None:
            data_list = ()
        else:
        ...
        for ident, data in data_list:
            ...
            if use_objects:
                self._store.add_data_to_state(pool, new_state, ident, data)

            elif log_exceptions:
                try:
                    self._store.create_new_data(
                        pool,
                        new_state,
                        data_catalog,
                        data,
                    ...
                    e = sys.exc_info()[0]
                    msg = ("Reading variable {} generated error: {}").format(
                        ident,
                        e,
                    )
                ...
                self._store.create_new_data(
                    pool, new_state, data_catalog, data, metadata
                )
    ...
    def serialise_states(
        ...
        if simulation._merged_state is not None:
            safe_id = get_unique_id(used_identifiers)
            state_box = self._convert_state_to_box(
                simulation._merged_state, safe_id, state_dir, root_dir
            )
    ...
    def deserialise_states(self, simulation: Simulation, root_dir=None):
        ...
        if simulation._merged_state is not None:
            state = self._convert_box_to_state(
    ...
    def _get_active_inputs(self, pool, simulation, input_declaration):
        ...
        for declared_input in input_declaration:
            if isinstance(declared_input, str):
                ...
                log_msg = ("Checking mask status for input " '"{}".').format(
                    declared_input.variable_id
                )
                module_logger.debug(log_msg)
                ...
                if merged_state.has_index(declared_input.unmask_variable):
                    ...
                    data_value = self.get_data_value(
                        pool, simulation, declared_input.unmask_variable
                    )
    ...
    def _merge_active_states(
        ...
        for state in active_states:
            ...
            merged_map = self._update_dict(
                merged_map, data_map, remove_none_keys
            )
    ...
    def _update_dict(self, old_dict, new_dict, remove_none_keys=False):
        if remove_none_keys:
            ...
            final_dict = {
                k: old_dict[k] for k in old_dict if k not in remove_keys
            }
    ...
    def _convert_box_to_state(self, serial_box, root_dir=None):
        if not isinstance(serial_box, SerialBox):
            errStr = (
                "Only SerialBox objects are accepted. Passed type was {}"
        ...
        if "masked" in dump_dict and dump_dict["masked"]:
            new_state.mask()
...
class Controller(Loader):
    ...
    def copy_simulation(
        ...
        new_simulation = _copy_sim_class(simulation, force_title, null_title)

        # Copy hubs
        _copy_sim_hubs(simulation, new_simulation)

        # Copy the active states
        active_states = self._copy_active_sim_states(
            simulation, compact_none_states
        )
        ...
        new_simulation = self._merge_and_stamp(
            simulation, new_simulation, no_merge
        )
    ...
    def import_simulation(
        ...
        for state in active_states:
            new_state = self._store.import_datastate(src_pool, dst_pool, state)
            new_simulation.add_state(new_state)
    ...
    def get_interface_obj(self, simulation, hub_id, interface_name):
        hub = simulation.get_hub(hub_id)
        interface_cls_name = self.get_interface_cls_name(
    ...
    def mask_states(
        self, simulation, search_str=None, mask_after=None, no_merge=False
    ):
        n_masks = simulation.mask_states(search_str, mask_after)

        if no_merge or n_masks == 0:
            return n_masks
    ...
    def unmask_states(self, simulation, search_str=None, no_merge=False):
    ...
    def delete_masked_states(self, pool, simulation, no_merge=False):
        # Kill masked states
    ...
    def get_input_status(
        ...
        log_msg = ("Getting input status for interface " '"{}".').format(
            interface_name
        )
        module_logger.debug(log_msg)
        ...
        (input_declaration, optional_inputs) = interface_obj.get_inputs()

        all_inputs = self._get_active_inputs(
            pool, simulation, input_declaration
        )
        ...
        if hub.force_completed or hub.is_completed(interface_cls_name):
            ...
            new_input_status = {
                input_id: "unavailable" for input_id in all_inputs
            }
    ...
    def get_output_status(
        ...
        log_msg = ("Getting output status for interface " '"{}".').format(
            interface_name
        )
        module_logger.debug(log_msg)
        ...
        output_status = {
            output_id: "unavailable" for output_id in output_declaration
        }
        ...
        if exectuted_outputs is None:
            exectuted_outputs = []
        completed_interfaces = hub.get_completed_map()

        if force_last_completed is not None:
            lc_cls_name = self.get_interface_cls_name(
                simulation, hub_id, force_last_completed
            )
        ...
        if (
            ...
            if end_index is not None:
                exectuted_outputs = _get_executed_outputs(
    ...
    def get_data_value(
        self, pool, simulation, data_identity, level=None, check_identity=False
    ):
        if level is not None:
            read_simulation = deepcopy(simulation)
            self._mask_after_level(read_simulation, level)
        else:
            read_simulation = simulation

        if check_identity and not self.has_data(read_simulation, data_identity):
            return None

        data_value = super(Controller, self).get_data_value(
            pool, read_simulation, data_identity
        )
    ...
    def get_level_values(
        ...
        if levels is None:
            levels = OrderedSet(simulation.get_active_levels())

        level_results = OrderedDict()

        for level_key in levels:
            self._mask_after_level(simulation_copy, level_key, force_masks)

            if not self.has_data(simulation_copy, data_identity):
                continue

            level_value = self.get_data_value(
                pool, simulation_copy, data_identity
            )
    ...
    def input_available(
        ...
        result = super(Controller, self).input_available(
            pool, simulation, interface_obj, check_id
        )

        return result

    def can_load(self, pool, simulation, hub_id, interface_name):
        interface_obj = self.get_interface_obj(
            simulation, hub_id, interface_name
        )

        result = super(Controller, self).can_load(
            pool, simulation, interface_obj
        )
    ...
    def load_interface(
        ...
        interface_obj = super(Controller, self).load_interface(
            pool, simulation, interface_obj, skip_vars
        )
    ...
    def _compact_none_states(self, state_list):
        ...
        for state in state_list:
            if state.get_level() is None:
                ...
                if state.ismasked():
                    errStr = (
                        "State list can not be compacted if states "
    ...
    def _make_compact_state(self, state_list, level=None):
        ...
        for state in state_list:
            data_map = state.mirror_map()
            merged_map = self._update_dict(merged_map, data_map)

        compact_state = self._store.create_new_datastate(
            level=level, force_map=merged_map
        )

        return compact_state

    def _merge_and_stamp(self, old_simulation, new_simulation, no_merge=False):
        if not no_merge:
            # Update the merged state stored in the simulation
    ...
    def _mask_after_level(self, simulation, level, force_masks=None):
        ...
        self.unmask_states(simulation, no_merge=True)

        # Mask all output states after the given level
        self.mask_states(simulation, mask_after=level, no_merge=True)
        ...
        if force_masks is not None:
            for mask in force_masks:
                self.mask_states(simulation, search_str=mask, no_merge=True)

            self.unmask_states(simulation, search_str=level, no_merge=True)
...
def _copy_sim_hubs(old_simulation, new_simulation):
