...
from ..utilities.plugins import Plugin, create_object_list
...
class DataValidation(Plugin):
    ...
    def obj_from_metadef_dict(self, metadef_dict):
        ...
        if required_attrs - metadef_keys:
            errStr = (
                'Dictionary keys "{}" do not match MetaData '
                'attributes "{}". Aborting.'
            ).format(" ,".join(metadef_keys), " ,".join(required_attrs))
...
class DataStorage(Plugin):
    ...
    def __init__(self, definition_module, super_cls="Structure"):
        self._structures = self._init_structures(definition_module, super_cls)

    def _init_structures(self, definition_module, super_cls="Structure"):
        log_msg = "Searching for {} classes".format(super_cls)
        module_logger.debug(log_msg)

        cls_map = self._discover_plugins(definition_module, super_cls)
    ...
    def create_new_datastate(self, level=None, force_map=None):
        ...
        if level is None:
            log_msg = "DataState created".format()
        else:
    ...
    def import_datastate(self, src_pool, dst_pool, datastate, level=None):
        ...
        for data_identifier, data_index in data_map.items():
            ...
            if (
                dst_structure_name is not None
                and dst_structure_name == src_structure_name
            ):
                ...
                try:
                    ...
                    msgStr = (
                        "Comparison of data with identifier {} failed "
                        "with an unexpected error:"
                        "\n{}"
                    ).format(data_identifier, traceback.format_exc())
    ...
    def add_data_to_state(
        ...
        if not hasattr(datastate, "add_index"):
            errStr = (
                "Given datastate is of wrong type. Expected DataState "
                "but recieved {}."
        ...
        if data_obj is None:
            ...
            log_msg = ('New "{}" data stored with index ' "{}").format(
                data_identifier, data_index
            )
            module_logger.info(log_msg)

        if (
            datastate.has_index(data_identifier)
            and datastate.get_index(data_identifier) is not None
        ):
    ...
    def remove_data_from_state(self, pool, datastate, data_identifier):
        ...
        log_msg = (
            'Removing "{}" data stored with index ' "{} from state"
        ...
        if not pool.has_link(data_index):
            ...
            log_msg = (
                'Removing "{}" data stored with index ' "{} from pool"
            ).format(data_identifier, data_index)
            module_logger.info(log_msg)
    ...
    def create_new_data(
        ...
        if data_form not in self._structures:
            errStr = (
                'Unrecognised structure "{}" for data identifier: ' "{}"
    ...
    def get_data_value(self, data_pool, datastate, data_identifier):
        if not self.has_data(datastate, data_identifier):
            errStr = (
                "Data with identifier {} is not contained in the "
                "given datastate"
        ...
        log_msg = (
            'Retrieving "{}" data stored with index ' "{} from pool"
        ).format(data_identifier, data_index)
        module_logger.debug(log_msg)
    ...
    def create_pool_subset(self, data_pool, datastate):
        ...
        for var_id in var_ids:
            ...
            self.add_data_to_state(
                new_pool, new_datastate, var_id, new_data_obj
            )
    ...
    def is_valid(cls, data_catalog, variable_id):
        ...
        if variable_id in catalog_vars:
            result = True

        return result

    def _copy_datastate_meta(self, datastate, level=None):
        if level is None:
            level = datastate.get_level()

        new_datastate = self.create_new_datastate(level)
        if datastate.ismasked():
            new_datastate.mask()
    ...
    def _convert_data_to_box(
        ...
        if isinstance(data_obj, SerialBox):
            return
        ...
        try:
            ...
            msgStr = (
                "Saving of data with index {} failed with an unexpected "
                "error:\n{}"
    ...
    def _convert_box_to_data(
        ...
        if data_obj is None:
            ...
            data_obj = Data(data_box.identifier, structure_name, None)
    ...
    def _make_data(self, data_catalog, identifier, data, warn_missing=False):
        ...
        if metadata.structure not in self._structures:
            errStr = (
                'Unrecognised structure "{}" for data identifier: ' "{}"
            ).format(metadata.structure, identifier)
            raise KeyError(errStr)
