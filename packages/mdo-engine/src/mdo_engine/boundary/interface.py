...
class Interface(ABC):
    ...
    @classmethod
    @abc.abstractmethod
    def declare_inputs(cls) -> Optional[list[str]]:
    ...
    @classmethod
    @abc.abstractmethod
    def declare_outputs(cls) -> Optional[list[str]]:
    ...
    @classmethod
    @abc.abstractmethod
    def declare_optional(cls) -> Optional[list[str]]:
    ...
    def init_maps(self):
        ...
        self.data = {key: None for key in all_keys}
    ...
    def get_inputs(cls, drop_masks=False):
        ...
        all_optional = cls.declare_optional()

        if all_optional is None:
            all_optional = set()
        else:
            all_optional = set(all_optional)
    ...
    def _check_optional_valid(self):
        ...
        if input_identifiers is None:
            input_identifiers = []
        if optional_indentifiers is None:
            return
        ...
        if err_mapped:
            ...
            errStr = (
                "The following identifiers are declared optional  "
    ...
    def _get_all_ids(cls):
        all_inputs = cls.declare_inputs()
        all_outputs = cls.declare_outputs()
...
class MapInterface(Interface):
    ...
    def init_maps(self):
        ...
        for local, universal in id_map.items():
            if "." in local:
                errStr = (
                    "The character '.' may not be included in id_map "
                    "key for variable {}"
                ).format(universal)
    ...
    def _check_map_valid(self):
        ...
        if not_mapped:
            ...
            errStr = (
                "The following identifiers have not been mapped in "
                "interface {}: {}"
            ).format(self.get_name(), bad_ids_str)
        ...
        if dupes:
            ...
            errStr = (
                "The following identifiers have multiple mappings in "
                "interface {}: {}"
            ).format(self.get_name(), bad_ids_str)
        ...
        if err_mapped:
            bad_ids_str = ", ".join(err_mapped)
            errStr = (
                "The following identifiers have been erroneously "
                "mapped in interface {}: {}"
            ).format(self.get_name(), bad_ids_str)
    ...
    def safe_connect(self, attempts=3):
        ...
        while attempts:
            ...
            try:
                ...
                if (
                    hasattr(exc, "connection_invalidated")
