...
class DataDefinition(ABC):
    ...
    def _get_yaml_local_paths(self):
        ...
        if self.local_yaml_dir is None:
            return []
    ...
    def _get_yaml_user_paths(self):
        ...
        if self.user_yaml_dir is None:
            return []

        local_dir = UserDataPath(self.package_name, self.company_name)
...
class Structure(ABC):
    ...
    def __call__(self, data):
        ...
        if isinstance(data, tuple(self.immutables)):
            ...
            errStr = (
                "Value returned from structure {} must not be the "
                "original data"
