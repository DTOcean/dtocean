...
class DataCatalog:
    ...
    def filter_by_meta(self, meta_attr, value):
        ...
        for var_id, meta in self._metadata_variable_map.items():
            ...
            if (
                is_list(meta_value) and value in meta_value
            ) or meta_value == value:
...
class BaseState:
    ...
    def _init_data(self, data_map):
        ...
        if not all(isinstance(x, (str, type(None))) for x in data_map.values()):
...
class PseudoState(BaseState):
    ...
    def __init__(self, data_map, level=None):
