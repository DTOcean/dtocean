...
class Hub:
    ...
    def get_preceding_interfaces(
        self, interface_cls_name, ignore_completed=False
    ):
    ...
    def get_upcoming_interfaces(
        self, interface_cls_name, ignore_sequenced=False
    ):
        # If the hub has no ordering then return an empty dictionary
        if not self.has_order:
            return {}
...
class Pipeline(Hub):
    ...
    def set_completed(self, interface_cls_name=None):
        ...
        if (
            interface_cls_name is not None
