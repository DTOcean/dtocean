...
class Simulation:
    ...
    def add_state(self, datastate, overwrite=False):
    ...
    def mask_states(self, search_str=None, mask_after=None):
        ...
        if mask_after is not None:
            active_observed, active_start = self._get_start_index(
                self._active_states, mask_after
            )

            redo_observed, redo_start = self._get_start_index(
                self._redo_states, mask_after, list_reversed=True
            )
        ...
        if active_observed and not redo_observed and active_start is not None:
            (self._active_states, local_count) = self._mask_state_list(
                self._active_states, search_str, active_start
            )

            mask_count += local_count

        if active_observed and not redo_observed:
            (self._redo_states, local_count) = self._mask_state_list(
                self._redo_states, search_str, list_reversed=True
            )
        ...
        if redo_observed and redo_start is not None:
            (self._redo_states, local_count) = self._mask_state_list(
                self._redo_states, search_str, redo_start, list_reversed=True
            )
        ...
        for state in self._active_states:
            ...
            if search_str is None or (
                ...
                if state_level is None:
                    ...
                    log_msg = ("Unmasking DataState at level " '"{}".').format(
                        state.get_level()
                    )
    ...
    def pop_masked_states(self):
        delete_active = [
            state for state in self._active_states if state.ismasked()
        ]
        new_active = [
            state for state in self._active_states if not state.ismasked()
        ]

        delete_redo = [state for state in self._redo_states if state.ismasked()]
        new_redo = [
            state for state in self._redo_states if not state.ismasked()
        ]
    ...
    def get_all_levels(self, show_none=False, show_masked=True):
    ...
    def get_last_level(self, show_none=False, show_masked=True):
        if not self._active_states:
            result = None
    ...
    def _get_start_index(self, state_list, mask_after, list_reversed=False):
        observed = False
        start_index = None
    ...
    def _mask_state_list(
        self, state_list, search_str=None, start_index=0, list_reversed=False
    ):
        if not state_list:
            return state_list, 0
        ...
        for state in iter_states:
            ...
            if search_str is None or (
                ...
                if state_level is None:
                    ...
                    log_msg = ("Masking DataState at level " '"{}".').format(
                        state.get_level()
                    )
