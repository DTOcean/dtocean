    ...
    for identifier, row in root_sheet.iterrows():
        ...
        for key, df in sheets.items():
            ...
            if erroneous_indices:
                ...
                errStr = (
                    "The following erroneous indentifiers were found in "
                    "the {} sheet of file {}: {}"
                ).format(key, xl_path, erroneous_str)
            ...
            if identifier in df.index:
                ...
                try:
                    ...
                    errStr = (
                        "The following AttributeError occurred reading "
...
def dds_merge(original_list, update_list, verbose=False):
    ...
    for identifier, update in update_dict.items():
        ...
        if identifier in original_dict:
            ...
            if verbose:
                print("Added indentifier {}".format(identifier))

            merged_list.append(update)

    for identifier, original in original_dict.items():
        if verbose:
            print("Indentifier {} is missing".format(identifier))
...
def xl_to_data_yaml(
    ...
    data = xl_to_dds(xl_path, ignore_column, ignore_missing, sanitise_keys)
...
def dds_to_xl(dds_list, xl_path, root_cols=None):
    ...
    for sheet, df_dict in df_dict.items():
        if sheet == "ROOT" and root_cols is not None:
            df = pd.DataFrame(df_dict, columns=root_cols)
...
def xl_merge(
    ...
    if preserve_order:
        root_df = pd.read_excel(original_xl_path, sheet_name="ROOT")
...
def bootstrap_dds_interface():
    ...
    parser.add_argument(
        "xldir", help=("path to directory containing XL files"), type=str
    )
...
def xl_merge_interface():
    ...
    parser.add_argument(
        "original", help=("the path to the original XL DDS file"), type=str
    )

    parser.add_argument(
        "updated", help=("the path to the updated XL DDS file"), type=str
    )
