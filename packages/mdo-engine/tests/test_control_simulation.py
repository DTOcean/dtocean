...
import mdo_engine.test.data as data_plugins
...
from mdo_engine.control.simulation import Controller, _copy_all_sim_states
...
def test_add_datastate(controller):
    ...
    validation.update_data_catalog_from_definitions(catalog, data_plugins)
    ...
    series = controller.get_data_value(pool, new_sim, "site:wave:dir")
...
def test_copy_simulation(controller):
    ...
    copy_sim = controller.copy_simulation(pool, new_sim, "Fork Off!")
    ...
    series = controller.get_data_value(pool, copy_sim, "site:wave:dir")
...
def test_import_simulation_from_clone(controller):
    ...
    copy_sim = controller.import_simulation(
        src_pool, pool, new_sim, "Fork Off!"
    )
...
def test_import_simulation_from_new(controller):
    ...
    controller.add_datastate(
        src_pool, src_sim, "executed", catalog, [test_variable], [raw_data]
    )

    assert check_integrity(src_pool, [src_sim])

    copy_sim = controller.import_simulation(
        src_pool, pool, src_sim, "Fork Off!"
    )
...
def test_remove_simulation(controller):
    ...
    controller.remove_simulation(pool, copy_sim)
...
def test_convert_state_to_box(controller, tmpdir):
    ...
    test_state = new_sim._active_states[-1]
    state_box = controller._convert_state_to_box(
        test_state, "test", str(tmpdir)
    )
...
def test_convert_state_to_box_root(controller, tmpdir):
    ...
    test_state = new_sim._active_states[-1]
    state_box = controller._convert_state_to_box(
        test_state, "test", str(tmpdir), str(tmpdir)
    )
...
def test_convert_box_to_state_root(controller, tmpdir):
    ...
    loaded_state = controller._convert_box_to_state(state_box, str(tmpdir))
...
def test_copy_all_sim_states(controller):
    ...
    for old_state, copy_state in zip(old_states, copy_states):
        assert set(old_state.get_identifiers()) == set(
            copy_state.get_identifiers()
        )
...
def test_make_compact_state_same_var(controller):
    ...
    controller.add_datastate(
        pool, new_sim, None, catalog, [test_variable], [[1]]
    )

    controller.add_datastate(
        pool, new_sim, None, catalog, [test_variable], [[2]]
    )

    controller.add_datastate(
        pool, new_sim, None, catalog, [test_variable], [[3]]
    )
...
def test_make_compact_multi_vars(controller):
    ...
    controller.add_datastate(pool, new_sim, None, catalog, [var1], [[1]])

    controller.add_datastate(pool, new_sim, None, catalog, [var2], [[2]])

    controller.add_datastate(pool, new_sim, None, catalog, [var1], [[3]])
...
def test_compact_none_states(controller):
    ...
    controller.add_datastate(pool, new_sim, None, catalog, [var1], [[2]])

    controller.add_datastate(pool, new_sim, "level1", catalog, [var1], [[3]])

    controller.add_datastate(pool, new_sim, None, catalog, [var1], [[1]])

    controller.add_datastate(pool, new_sim, None, catalog, [var1], [[2]])

    controller.add_datastate(pool, new_sim, "level2", catalog, [var1], [[3]])
