...
class Visualiser:
    ...
    def show_k_fit(self, te_i, hs_i, ang_i):
        ...
        ax.set_xticklabels(
            ["{:.1f}".format(tp) for tp in range(dofi)], fontsize=sz_ticks
        )
        ax.set_xlabel("dof i ", fontsize=sz_labels)
        ax.xaxis.set_label_position("top")
        ax.set_yticks(range(dofj))
        ax.set_yticklabels(
            ["{:.1f}".format(hs) for hs in range(dofj)], fontsize=sz_ticks
        )
    ...
    def show_c_fit(self, te_i, hs_i, ang_i):
        ...
        cb = Figure.colorbar(fig, mtsh)
        cb.set_label(
            "Fitted Damping (N/(m/s) [Nm/(rad/s)]) ", fontsize=sz_labels
        )
        ax.grid(False)
        return fig
    ...
    def show_original_power_matrix(self, ang_i):
        ...
        ax.set_xticks(range(dofi))
        ax.set_xticklabels(
            ["{:.1f}".format(te) for te in self._data_p["te"]],
            fontsize=sz_ticks,
        )
        ...
        ax.set_yticks(range(dofj))
        ax.set_yticklabels(
            ["{:.1f}".format(hs) for hs in self._data_p["hm0"]],
            fontsize=sz_ticks,
        )
        ax.set_ylabel("Hm0", fontsize=sz_labels)
        cb = Figure.colorbar(fig, mtsh)
        cb.set_label(
            "Power (W) - wave dir {}deg".format(
                self._data_p["wave_dir"][ang_i] * 180 / np.pi
            ),
            fontsize=sz_labels,
        )
        ax.grid(False)
