...
from . import mesh as _msh


def Hydrostatics_Nemohcal(directory):
    """ """
    (mesh_files, modes) = ReadNemohcal_mshs_mds(directory)
    meshes = [_msh.readDAT(mesh_file) for mesh_file in mesh_files]
    for mesh in meshes:
        mesh.refine_mesh(option=2, triangles=False)
    cb = (
        [
            [
                False,
...
def ReadNemohcal_mshs_mds(directory):
    ...
    with open(os.path.join(directory, "Nemoh.cal"), "r") as f_N:
        ...
        for body in range(Nb):
            ...
            mesh_file = mesh_file.replace("'", "")
            if mesh_file[0] == ".":
                mesh_files.append(directory + mesh_file[1:])
            elif mesh_files[0][0] == os.path.join("a", "a")[1]:
                mesh_files.append(directory + mesh_file)
            else:
                mesh_files.append(os.path.join(directory, mesh_file))
            Ndof = int(content[strt + 2].split("!")[0])
            DoF = [
                np.array(content[strt + 3 + ndof].split("!")[0].split(), float)
                for ndof in range(Ndof)
            ]
            Nforce = int(content[strt + 3 + Ndof].split("!")[0])
            FoRcE = [
                np.array(
                    content[strt + 4 + Ndof + nforce].split("!")[0].split(),
                    float,
                )
                for nforce in range(Nforce)
            ]
            modes.append([DoF, FoRcE])
            Nadd = int(content[strt + 4 + Ndof + Nforce].split("!")[0])
            strt += 5 + Ndof + Nforce + Nadd
    return (mesh_files, modes)


def Hydrostatics(
    meshes: Sequence[Union[_msh.SurF, str]],
    modes,
    cb,
    cg,
    ro=1000.00,
    ...
    for nb, mesh in enumerate(meshes):
        ...
        if isinstance(mesh, str):
            if mesh[-3:] == "GDF" or mesh[-3:] == "gdf":
                ...
                raise ValueError("Mesh file type not recognised")
        ## center of gravity
        mesh.NCA()
        if len(cg[nb]) == 3:
            xg = cg[nb]
        ...
        kbuoy, kgrav = (
            np.zeros((FORCE, DOF), dtype=float),
            np.zeros((FORCE, DOF), dtype=float),
        )
        fbuoy, fgrav = (
            np.zeros((FORCE, DOF), dtype=float),
        ...
        for do, dof in enumerate(modes[nb][0]):
            for fo, force in enumerate(modes[nb][1]):
                kbuoy[fo, do], fbuoy[fo, do] = ForceB(
                    dof, force, Vol, xb, S, fixedA=fixedA
                )
                kgrav[fo, do], fgrav[fo, do] = ForceG(
                    dof, force, xg, fixedA=fixedA
                )
        ##
    ...
    if not moment:
        ...
        if not rot:
            ...
            if fixedA:
                K[0] = (
                    axd[0]
                    * (-(zb - zB) * Vol + (Syy - (yB + yA) * Sy + yB * yA * Sw))
                    - axd[1] * (Sxy - yA * Sx - xB * Sy + xB * yA * Sw)
                    + axd[2] * (xb - xB) * Vol
                )  # axd[0]*(-(zb-zB)*Vol+np.dot((y-yB)*(y-yA), nz*Aw)) - axd[1]*np.dot((x-xB)*(y-yA), nz*Aw) + axd[2]*(xb-xB)*Vol
                K[1] = (
                    -axd[0] * (Sxy - xA * Sy - yB * Sx + yB * xA * Sw)
                    + axd[1]
                    * (-(zb - zB) * Vol + (Sxx - (xB + xA) * Sx + xB * xA * Sw))
                    + axd[2] * (yb - yB) * Vol
                )  # -axd[0]*np.dot((y-yB)*(x-xA), nz*Aw) + axd[1]*(-(zb-zB)*Vol+np.dot((x-xB)*(x-xA), nz*Aw)) + axd[2]*(yb-yB)*Vol
            else:
                K[0] = (
                    axd[0]
                    * (-(zb - zA) * Vol + (Syy - (yB + yA) * Sy + yB * yA * Sw))
                    - axd[1] * (Sxy - yA * Sx - xB * Sy + xB * yA * Sw)
                    + axd[2] * (xb - xA) * Vol
                )  # axd[0]*(-(zb-zA)*Vol+np.dot((y-yB)*(y-yA), nz*Aw)) - axd[1]*np.dot((x-xB)*(y-yA), nz*Aw) + axd[2]*(xb-xA)*Vol
                K[1] = (
                    -axd[0] * (Sxy - xA * Sy - yB * Sx + yB * xA * Sw)
                    + axd[1]
                    * (-(zb - zA) * Vol + (Sxx - (xB + xA) * Sx + xB * xA * Sw))
                    + axd[2] * (yb - yA) * Vol
                )  # -axd[0]*np.dot((y-yB)*(x-xA), nz*Aw) + axd[1]*(-(zb-zA)*Vol+np.dot((x-xB)*(x-xA), nz*Aw)) + axd[2]*(yb-yA)*Vol
            f[0] = -(yb - yA) * Vol
            f[1] = (xb - xA) * Vol
