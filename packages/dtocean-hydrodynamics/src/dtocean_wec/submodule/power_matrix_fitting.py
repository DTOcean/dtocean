...
class PowerMatrixFit:
    ...
    def perf_fitting(self, machine_spec, site_spec):
        ...
        wdirs_yaw = set_wdirs_with_yaw(
            wave_angles.copy(), sp_spreading, m_angle, y_angle_range
        )
        ...
        self.c_ext = self.__check_matrix_dimension(
            self.c_ext, sd_shape, range(ndof)
        )
        self.k_ext = self.__check_matrix_dimension(
            self.k_ext, sd_shape, range(ndof)
        )
        CPTO_sm = self.__check_matrix_dimension(
            machine_spec["c_pto"], sd_shape, self.pto_dof
        )  # pto damping matrix for the single machine
        ...
        Kmooring_sm = self.__check_matrix_dimension(
            machine_spec["k_mooring"], sd_shape, self.moor_dof
        )  # mooring stiffness damping matrix for the single machine
        # overrule the user input by setting the index priority to the one read in the gnr file
        mask = np.ones_like(Kmooring_sm, dtype=bool)
        mask[:, :, :, self.moor_dof, self.moor_dof] = False
        Kmooring_sm[mask] = 0.0
        ...
        if np.all(machine_spec["power_matrix"] == 0):
            self.c_fit = Cfit_sm
            self.k_fit = Kfit_sm
            if self.force_matrix.shape == (1,):
                self.wec_power_matrix = self.__get_wec_power_matrix(
                    wdirs_yaw, wec_power_matrix, no_yaw=True
                )
            else:
                self.wec_power_matrix = self.__get_wec_power_matrix(
                    wdirs_yaw, wec_power_matrix
                )
        ...
        Nm = np.array(
            range(-self.order.max(), self.order.max() + 1), dtype=float
        )
            ...
            for t_ind in range(len(te)):
                for d_ind in range(len(wave_angles)):
                    ...
                    for orient, thetas in zip(wdirs_yaw[::2], wdirs_yaw[1::2]):
                        ...
                        Kfit_sm[t_ind, h_ind, d_ind] = np.diag(
                            lin_fit_b[0][ndof:]
                        )
                        wec_power_matrix[t_ind, h_ind, d_ind] = (
                            self.__power_fit(
                                lin_fit_b[0],
                                (t_ind, h_ind, d_ind),
                                thetas,
        ...
        self.wec_original_power_matrix = self.__get_wec_power_matrix(
            wdirs_yaw, wec_original_power_matrix, skip_fit=True
        )
    ...
    def __power_fit(self, x, seastate_id, thetas, target_pow, f_ex):
        ...
        """
        assert self.c_pto is not None
        assert self.k_mooring is not None
        assert self.k_ext is not None
        assert self.c_ext is not None
        assert self.wave_dir is not None
        assert self.hm0 is not None
    ...
    def __get_wec_power_matrix(
        ...
        assert self.wave_dir is not None
        assert self.hm0 is not None
        assert self.te is not None
        assert self.c_fit is not None
        assert self.k_fit is not None
        ...
        if no_yaw:
            ...
            for h_ind in range(len(self.hm0)):
                for t_ind in range(len(self.te)):
                    for d_ind in range(len(self.wave_dir)):
                        ind = np.argmin(
                            np.abs(self.wave_dir[d_ind] - self.directions)
                        )
                        f_ex = self._fex[:, ind, :].reshape(
                            (len(self.periods), 1, self.n_dof)
                        )
                        x = np.r_[
                            np.diag(self.c_fit[t_ind, h_ind, d_ind]),
                            np.diag(self.k_fit[t_ind, h_ind, d_ind]),
                        ]
                        wec_power_matrix[t_ind, h_ind, d_ind] = (
                            self.__power_fit(
                                x * x_scale,
            ...
            for h_ind in range(len(self.hm0)):
                for t_ind in range(len(self.te)):
                    for d_ind in range(len(self.wave_dir)):
                        for orient, thetas in zip(
                            wdirs_yaw[::2], wdirs_yaw[1::2]
                        ):
                            if self.wave_dir[d_ind] not in thetas:
                                continue
                            ths = np.array([th for th in thetas], dtype=float)
                            g_rot = np.transpose(
                                Gp * np.exp(-1j * Nm * orient), axes=(0, 2, 1)
                            ...
                            AP = np.exp(-1j * mode * (np.pi / 2 + mths))
                            f_ex = np.zeros(
                                (len(self.periods), len(thetas), self.n_dof),
                                dtype=complex,
                            )
                            for f in range(len2(self.periods)):
                                f_ex[f, :, :] = np.dot(AP, g_rot[f])
