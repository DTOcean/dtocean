...
class NemohExecute:
    ...
    def __init__(
        ...
        if (
            self.Cyl_Nzeta == 0
            or self.Cyl_Ntheta == 0
            or not self.__get_array_mat
        ):
            self.Cyl_radius = 0
        else:
    ...
    def gen_multibody_structure(self):
        ...
        self.bodies = (
            mb_obj.bodies
        )  # apply the new bodies numbering to the nemoh_obj
    ...
    def gen_hdyn_files(self):
        ...
        f.write(
            "{:.2f}				! RHO 			! KG/M**3 	! Fluid specific volume \n".format(
                self.rho
            )
        )
        f.write(
        ...
        f.write(
            "{}				! DEPTH			! M		! Water depth\n".format(
                abs(self.water_depth)
            )
        )  # the water depth cannot be infinity
        f.write(
        ...
        for nb in range(self.n_bodies):
            ...
            f.write(
                "{}\t{}			! Number of points and number of panels \n".format(
                    int(self.bodies[nb]["nV"]), int(self.bodies[nb]["nP"])
                )
            )
            f.write(
            ...
            for dof in self.bodies_dofs[nb]:
                f.write(
                    "{:g} {} {} {} {} {} {}		! DOF type(1x1) direction(3x1) position(3x1)\n".format(
                        *dof
                    )
                )

            f.write(
                "{}				! Number of generalised forces\n".format(
                    n_sh
                )
            )
        ...
        f.write(
            "{} {} {}		! Number of wave angles, Min, and Max (deg)\n".format(
                self.angle_def[0], self.angle_def[1], self.angle_def[2]
            )
        )
        f.write(
            "--- Post processing ---------------------------------------------------------------------------------------------------------------\n"
        ...
        f.write(
            "{}     {}     {}                                ! Cylindrical surface     ! Radius (0 for no calc), Number of points in theta direction, Number of points in z direction\n".format(
                self.Cyl_radius, self.Cyl_Ntheta, self.Cyl_Nzeta
            )
        )
        f.write(
            " --- End of file -------------------------------------------------------------------------------------------------------------------\n"
...
def _get_cylinder_radius(meshes):
    ...
    for mesh in meshes:
        ...
        for panel in mesh.connectivity:
            d_panel = max(
                [
                    LA.norm(mesh.vertices[panel[0]] - mesh.vertices[panel[s]])
                    for s in [1, -1]
                ]
            )
