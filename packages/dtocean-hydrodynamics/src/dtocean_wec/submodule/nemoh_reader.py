...
from .utils import input_control as checker
...
class NemohReader:
    def __init__(
        ...
        if self.angle_def[1] == 0.0 and self.angle_def[2] == 360.0:
        ...
        self.loose_dof = len(self.bodies) * len(
    ...
    def map_generalise_dof(self, k_hst, c_rad, m_add, f_ex, phi_r):
        if self.vel_map is None:
            raise RuntimeError("Data not loaded. Call load_data method first")
    ...
    def get_cylinder_dimension(self):
        ...
        cylinder = f_util.split_string(lines[-i_lin_set - 2])
    ...
    def read_results(self):
        ...
        freq_ca, freq_cm, freq_fex, angle_fex = [
            vect.reshape(-1) for vect in [freq_ca, freq_cm, freq_fex, angle_fex]
        ]

        # perform some final check on the data format
        if not np.allclose(freq_ca, freq_cm) or not np.allclose(
            freq_cm, freq_fex, rtol=1e-3
        ):
            raise AttributeError(
                "The frequency retrieved from the nemoh results does not match each other.",
                "Check the consistency of the result folder",
            )

        if not np.allclose(freq_ca, 2.0 * np.pi / self.periods, rtol=1e-3):
            raise ValueError(
                "The frequency vector retrieved from the nemoh results does not match the one",
                " given in the input.",
            )

        if not np.allclose(angle_fex, self.directions, rtol=1e-3):
            raise ValueError(
                "The angle vector retrieved from the nemoh results does not match the one",
                " given in the input.",
            )
    ...
    def __read_ca(self):
        ...
        n_lines = int(np.ceil(self.loose_dof / 6.0))
    ...
    def __read_fex(self):
        f_Ex = open(
            os.path.join(self._path_prj_dyn_res, "ExcitationForce.tec"), "r"
        )
        ...
        n_lines = int(np.ceil(((self.loose_dof * 2) + 1) / 80.0))
        ...
        for i_angle in range(n_directions):
            ...
            for i_freq in range(n_freq):
                ...
                fex[i_freq, i_angle, :] = MagPha[0:-1:2] * np.exp(
                    1j * MagPha[1::2]
                )
    ...
    def __read_cylsurface(self):
        ...
        if not self.cyl_radius == 0.0:
            n_problems = self.loose_dof * n_freq + n_directions * n_freq

            Pressure = 1j * np.zeros(
                (self.cyl_ntheta, self.cyl_nzeta, n_problems)
            )
            ...
            for problems in range(n_problems):
                ...
                f_prob = open(
                    os.path.join(
                        self._path_prj_dyn_res,
                        "cylsurface.{:5d}.dat".format(problems + 1),
                    ),
                    "r",
                )
                f_prob.readline()
                ...
                for n_zeta in range(self.cyl_nzeta):
                    for n_theta in range(self.cyl_ntheta):
                        ...
                        Pressure[n_theta, n_zeta, problems] = temp[3] * np.exp(
                            1j * temp[4]
                        )
                f_prob.close()

            Scattering = 1j * np.zeros(
                (n_freq, n_directions, self.cyl_nzeta, self.cyl_ntheta)
            )
            Radiation = 1j * np.zeros(
                (n_freq, self.loose_dof, self.cyl_nzeta, self.cyl_ntheta)
            )
            ...
            for i_cfreq in range(n_freq):
                for i_angle in range(n_directions):
                    problem_index += 1
                    Scattering[i_cfreq, i_angle, :, :] = np.transpose(
                        Pressure[:, :, problem_index]
                    )
                for dof in range(self.loose_dof):
                    problem_index += 1
                    Radiation[i_cfreq, dof, :, :] = np.transpose(
                        Pressure[:, :, problem_index]
                    )
        else:
            # Dummy data when no field points are selected
            raise ValueError(
                "The get_array_matrix flag is true but the cylinder radius is zero. ",
        ...
        Scattering[np.isnan(Scattering)] = 0.0
        Radiation[np.isnan(Radiation)] = 0.0

        cfreq = 2.0 * np.pi / self.periods
        g = self.g
        for ind, freq in enumerate(cfreq):
            Scattering[ind] *= g / 1j / freq
            Radiation[ind] *= (
                g / 1j / freq * (-1j * freq)
            )  # *(-1j*freq) Radiation will be due to unit amplitude motion
