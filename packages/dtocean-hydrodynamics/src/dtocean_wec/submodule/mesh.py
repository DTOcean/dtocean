...
class SurF(object):
    ...
    def rotation(self, rot):
        ...
        R = [
            np.array(
                [[np.cos(angl), -np.sin(angl)], [np.sin(angl), np.cos(angl)]],
                dtype=float,
            )
            for angl in rot
        ]
    ...
    def Show_Norms(self, delta):
        ...
        for n, pan in enumerate(self.coord):
            ...
            ax.plot(
                [
                    delta * self.norm[n, 0] + self.center[n, 0],
                    self.center[n, 0],
                ],
                [
                    delta * self.norm[n, 1] + self.center[n, 1],
                    self.center[n, 1],
                ],
                [
                    delta * self.norm[n, 2] + self.center[n, 2],
                    self.center[n, 2],
                ],
                "b-",
            )
    ...
    def NCA(self):
        ...
        self.norm = (
            self.norm.T / np.sqrt(np.sum(self.norm * self.norm, axis=1))
        ).T
        ...
        for n, pan in zip(self.tri, self.coord[self.tri, :, :]):
            ...
            S1, S2, S3 = (
                np.sqrt(np.dot(S1, S1)),
                np.sqrt(np.dot(S2, S2)),
                np.sqrt(np.dot(S3, S3)),
            )
            s = (S1 + S2 + S3) * 0.5
    ...
    def Chkud(self, refpoints):
        ...
        for n, pan in enumerate(self.coord):
            ...
            if m1 < m2:
                ...
                if (
                    n in self.tri
                ):  # if it is a triangle the first nodes must be equal to the second
    ...
    def refine_mesh(self, option=2, triangles=False):
        ...
        # quad
        if option == 1:
            crd_ref[:stp:2] = np.transpose(
                np.array(
                    [crd_q[:, 0], crd_q[:, 0], crd_q[:, 1], crd_q[:, 2]], float
                ),
                axes=(1, 0, 2),
            )
            crd_ref[1:stp:2] = np.transpose(
                np.array(
                    [crd_q[:, 0], crd_q[:, 0], crd_q[:, 2], crd_q[:, 3]], float
                ),
                axes=(1, 0, 2),
            )
        else:
            crd_ref[:stp:4] = np.transpose(
                np.array([crd_q[:, 0], crd_q[:, 0], crd_q[:, 1], cen_q], float),
                axes=(1, 0, 2),
            )
            crd_ref[1:stp:4] = np.transpose(
                np.array([cen_q, cen_q, crd_q[:, 1], crd_q[:, 2]], float),
                axes=(1, 0, 2),
            )
            crd_ref[2:stp:4] = np.transpose(
                np.array([crd_q[:, 3], crd_q[:, 3], cen_q, crd_q[:, 2]], float),
                axes=(1, 0, 2),
            )
            crd_ref[3:stp:4] = np.transpose(
                np.array([crd_q[:, 0], crd_q[:, 0], cen_q, crd_q[:, 3]], float),
                axes=(1, 0, 2),
            )
        # tri
        if triangles:
            crd_ref[stp::3] = np.transpose(
                np.array([crd_t[:, 1], crd_t[:, 1], crd_t[:, 2], cen_t], float),
                axes=(1, 0, 2),
            )
            crd_ref[stp + 1 :: 3] = np.transpose(
                np.array([cen_t, cen_t, crd_t[:, 2], crd_t[:, 3]], float),
                axes=(1, 0, 2),
            )
            crd_ref[stp + 2 :: 3] = np.transpose(
                np.array([crd_t[:, 1], crd_t[:, 1], cen_t, crd_t[:, 3]], float),
                axes=(1, 0, 2),
            )
        self.coord = crd_ref
...
def mesher(
    Nth, Nz, radius, draught, geometry, thf=2 * pi, zcut=0.0, axes=(0, 1, 2)
):
    """
    ...
    ## Non circular sectional area
        ...
        for i in range(Nth - Nn):
            ...
            n0 = inds[: Nn + i - 1][L == L.max()][
                0
            ]  # initial node of the largest interval
            buff = (x[n0 + 1 : -1].copy(), y[n0 + 1 : -1].copy())
            if abs(x[n0 + 1] - x[n0]) < 1e-6:
                x[n0 + 1] = x[n0 + 1] / 2 - x[n0] / 2.0 + x[n0]
                y[n0 + 1] = y[n0 + 1] / 2 + y[n0] / 2.0 + y[n0]
            else:
    ...
    if (
        ...
        Npp = (
            (Nth - 1) / 2
        )  # number of panels that will be generated instead of the Nth-1 panels
        conp = np.array(
            [
                Id[-2, 2::2],
                Id[-2, 1:-1:2],
                Id[-2, :-2:2],
                Nth * (Nz - 1) * np.ones(Npp),
            ],
            dtype=int,
        ).T
        con = con[: Npp - (Nth - 1)]
    ...
    if geometry == "Hemisphere":
        if abs(draught) != abs(radius):
            print(
                "draught must be equal to the radius, i.e. draught == radius = True"
            )
    ...
    if geometry == "Sphere":
        if abs(draught) != abs(2 * radius):
            print(
                "draught must be equal to the diameter, i.e. draught == 2*radius = True"
            )
        tr = draught / 2
...
def circumference(Nl, radius, z, upwards=True):
    ...
    Xe1 = np.array(
        [xn[0], xn[1], xn[0], -bordx[0], xn[-1], xn[-2], xn[-1], bordx[0]],
        dtype=float,
    )
    Ye1 = np.array(
        [yn[0], bordy[0], yn[0], yn[1], yn[-2], yn[-1], yn[-1], -bordy[0]],
        dtype=float,
    )
    ...
    # assemble
    coord = z * np.ones(
        (coord0.shape[0] + coord1.shape[0] + coord2.shape[0], 4, 3)
    )
    coord[: coord0.shape[0], :, :2] = coord0[:, :, :2]
    coord[coord0.shape[0] : coord0.shape[0] + coord1.shape[0], :, :2] = coord1[
        :, :, :2
    ]
...
def readGDF(fn, translation=np.zeros(3), rotation=np.zeros(3)):
    ...
    with open(fn, "r") as fIlE:
        ...
        for panel in range(Npanels):
            for node in range(4):
                coord[panel, node] = np.array(
                    lines[panel * 4 + node].split(), dtype=float
                )
                ...
                if any(rotation != 0):
                    for d, ax in enumerate(((1, 2), (0, 2), (0, 1))):
                        R = np.array(
                            [
                                [np.cos(rotation[d]), -np.sin(rotation[d])],
                                [np.sin(rotation[d]), np.cos(rotation[d])],
                            ],
                            dtype=float,
                        )
                        coord[panel, node, ax] = np.dot(
                            R, coord[panel, node, ax]
                        )
    ## instantiate the surface using coord as (Npanels, Nnodes, Ncoord)
    surface = SurF(coord)
    return surface
...
def Bounds(point, coord, norm, center, area, betaf, alphai, alphaf, Nb, Na):
    ...
    for ip, pan in enumerate(coord):
        ...
        denominator = (
            norm[ip, 0] * spherical[0]
            + norm[ip, 1] * spherical[1]
            + norm[ip, 2] * spherical[2]
        )
...
def A4Pol(coord):
    ...
    for pivot in (abc, cda):
        aca = (
            0.5 / ac * (pivot[0] ** 2 + ac**2 - pivot[1] ** 2)
        )  # ac is divided into aca and acc so that aca + acc == ac
...
if __name__ == "__main__":
    ...
    beam.translation(np.array((-30.0, 0.0, 0.0)))
    hinge.translation(np.array((-5.0, 0.0, 0.0)))
    head.translation(np.array((-30.0, 0.0, 0.0)))
    ...
    pelamisR.rotation(np.array((pi, -pi, 0.0)))
    ...
    weird = mesher(
        Nth=25, Nz=10, radius=5, draught=5, geometry=geometry, thf=2 * pi
    )
