...
class MultiBodyAnalysis:
    def __init__(
        self,
        bodies_description: Sequence[dict],
        shared_dof_bin,
        local_cs,
    ):
    ...
    def set_multibody_data(self, second_stage=False):
        ...
        n_joints = (
            sum([el["number_child"] for el in bodies]) + 1
        )  # the +1 is due to the floating joint at the root
        joints_pos = [
            [
                -1,
                [
    ...
    def get_multibody_topology(self):
        ...
        preliminary_path_matrix = self.get_path_matrix(
            self.joints_pos, self._n_bodies, self._n_joints
        )

        # renumber the bodies in order to obtain an easier path_matrix
        o_bodies = self.ordering_multibody_tree(
            self.bodies, preliminary_path_matrix
        )
        ...
        path_matrix = self.get_path_matrix(
            self.joints_pos, self._n_bodies, self._n_joints
        )
    ...
    def set_joints_position_orientation(self):
        if self._n_joints is None:
            raise RuntimeError("Call set_multibody_data first")

        assert self.joints_pos is not None

        if self.igs is None:
            raise RuntimeError("Call get_multibody_topology first")
        ...
        p_partial = np.array([0, 0, 0.0])
        while j_c < j_m:
            node_i = np.where(Igs[:, edge_j] == -1)[0][0]
            if node_i == 0:
                j_c += 1
                edge_j = j_c + 0
                joints_pos_abs.append(
                    p_partial + np.asarray(self.joints_pos[0][1][1:])
                )
                p_partial = np.array([0, 0, 0.0])
            else:
                ...
                J_pos = next(
                    x for x in parent["child_dof_position"] if x[0] == node_i
                )
    ...
    def eval_velocity_transformation_matrix(self):
        ...
        if self.joints_pos_abs is None:
            raise RuntimeError("Call set_joints_position_orientation first")
        ...
        for edge in range(n_joints):
            ...
            for node in range(n_bodies):
                ...
                for j in range(joint_ind + 1):
                    if not isinstance(rr[node][j], np.ndarray):
                        continue
                ...
                if edge == 0:  # floating platform
                    Bij = np.c_[np.r_[Id, Z], np.r_[-self.tilde(dij), Id]]
                    # reduce the order accordlying to the real number of dofs of the floating
                    # platform
                    bij = Bij[
                        np.ix_(
                            np.where(shared_dof_bin)[0],
                            np.where(shared_dof_bin)[0],
                        )
                    ]
                    Bm[
                        loose_dof_b * node : loose_dof_b * (node + 1),
                        :loose_dof_b,
                    ] = bij
                else:
                    for idof, dof in enumerate(actual_joint_dof):
                        ...
                        b_col[
                            loose_dof_b * node : loose_dof_b * (node + 1), idof
                        ] = bij
    ...
    def get_dofs_floating_bodies(self):
        ...
        for ibi, bi in enumerate(self.bodies):
            ...
            body_dofs += [
                el[:4] + cog.tolist() for el in base_dofs if el[0] == 2
            ]
    ...
    def ordering_multibody_tree(bodies, path_matrix):
        ...
        root_ind = [
            ind for ind, el in enumerate(path_matrix) if np.all(el >= 0)
        ][0]
        n_bodies = len(bodies)
        ...
        while len(old_num) < n_bodies:
            ...
            for b_i in base_ind:
                child = np.where(
                    path_matrix[:, path_matrix[b_i, :] == 1] == -1
                )[0].tolist()
                old_num += child
                new_b_i += child
            ...
            if iteration > 1000:
                print(
                    "Ops... something went worng: could not find the multibody path matrix"
                )
                break
        ...
        for body in bodies:
            ...
            if isinstance(body_id, list):
                body_id = body_id[0]
            bn["ID"] = numb[1][
                [ii for ii, el in enumerate(numb[0]) if el == body_id][0]
            ]
            if bn["number_child"] > 0:
                ...
                for ch_p in old_pos:
                    ch_p[0] = numb[1][
                        [ii for ii, el in enumerate(numb[0]) if el == ch_p[0]][
                            0
                        ]
                    ]
                    new_pos.append(ch_p)
    ...
    def tilde(vec):
        tilde_mat = np.array(
            [[0, -vec[2], vec[1]], [vec[2], 0, -vec[0]], [-vec[1], vec[0], 0.0]]
        )
        return tilde_mat
