...
def set_wdirs_with_yaw(B, s, mA, yA, debug=False):
    ...
    yaw_ind = np.logical_or(
        rot_B >= angle_wrap(-(yA + 0.0001), "r2r"), rot_B <= yA
    )
    ...
    if not np.any(yaw_angles):
        ...
        if dir_spr:
            tuple_angles += tuple(
                [
                    angle_wrap(el - dir_spr / 3.0 * 2, "r2r")
        ...
        if (
            ...
            plusBound = np.argmin(
                np.abs(angle_wrap(yaw_angles - mA, "r2r") - yA)
            )
            minusBound = np.argmin(
                np.abs(
                    angle_wrap(yaw_angles - mA, "r2r") - angle_wrap(-yA, "r2r")
                )
            )
            ...
            if (
                ...
                max_range = tuple(
                    [angle_wrap(el + mA + np.pi, "r2r") for el in ang_distr]
                )
            else:
                max_range = tuple(
                    [
                        angle_wrap(el + mA + np.pi, "r2r")
                        for el in ang_distr
                        if el >= np.pi
                    ]
                )
