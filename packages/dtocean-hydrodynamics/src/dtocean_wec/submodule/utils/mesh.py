...
class MeshBem(object):
    ...
    def visualise_mesh(self, ax=None, fig=None):
        ...
        panels = [Panel(self.vertices[panel, :]) for panel in self.connectivity]
        ...
        if output_format == "gdf":
            ...
            with open(file_p, "w") as f:
                ...
                for panel in range(self.nP):
                    f.write(
                        int_format.format(*(self.connectivity[panel, :] + 1))
                    )
            ...
            raise ValueError(
                "Unsupported output format. Use gdf, nemoh or mesh"
            )
...
class Panel(object):
    ...
    def show_norm(self, ax):
        ...
        v24 = np.array([x[3] - x[1], y[3] - y[1], z[3] - z[1]])
        v31 = np.array([x[0] - x[2], y[0] - y[2], z[0] - z[2]])
...
def read_NEMOH(f_n):
    ...
    if len(first_line) == 1:
        ...
        for panel in range(nP):
            connectivity[panel, :] = (
                np.array(lines.pop(0).split(), dtype=float) - 1
            )
...
def read_WAMIT(f_n):
    ...
    points = np.array(
        [float(val) for entries in lines for val in entries.split()]
    )
...
def strip_comments(code):
    msg = re.sub(r"\s*!.*", "", code, flags=re.MULTILINE)
    msg = re.sub(r"^\n", "", msg)
