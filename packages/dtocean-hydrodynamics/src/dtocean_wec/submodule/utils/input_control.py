...
def check_hydrodynamic_files(folder, get_array_mat=True):
    ...
    for el in base_file_list:
        if not os.path.isfile(os.path.join(folder, el)):
            return (
                False,
                "Missing the {} file in the results folder".format(el),
            )
    ...
    with open(
        os.path.join(hdy_folder, "results", "ProblemDescription.txt")
    ) as fid:
...
def check_hydrostatic_files(folder, n_bodies):
    ...
    for body in range(n_bodies):
        if not os.path.isfile(
            os.path.join(hst_folder, "body{}".format(body), "mesh", "KH.dat")
        ):
...
def check_nemoh_results(data_folder, get_array_mat=True):
    ...
    status_dy_files = check_hydrodynamic_files(
        data_folder, get_array_mat=get_array_mat
    )
...
def check_wamit_results(data_folder, get_array_mat=True):
    ...
    check_file_type = [
        el for el in os.listdir(data_folder) if el.endswith(".6p")
    ]
    if get_array_mat and not check_file_type:
        return (
            False,
            "The specified filename (.6) does not exist in the folder {}".format(
                data_folder
            ),
        )
    elif (
        len(check_file_type) > 1
    ):  # this cndition cannot exist, just keep it here in case the comparison is bugged
        return (
            False,
            "The specified filename (.6) exists in multiple copies in the folder {}. ".format(
                data_folder
            ),
...
def check_filetype_in_folder(ftype, data_folder):
    check_file_type = [
        el for el in os.listdir(data_folder) if el.endswith(ftype)
    ]
