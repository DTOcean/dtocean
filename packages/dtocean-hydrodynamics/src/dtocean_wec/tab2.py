...
class RunNemoh(QWidget, Ui_T2):
    ...
    def check_body_data(self):
        ...
        dof_body = split_string_multilist(
            self.dof_body.text(), sep=",", sep_multi=";"
        )
        chil_body = split_string_multilist(
            self.chil_body.text(), sep=",", sep_multi=";"
        )
        parent_body = split_string(self.parent_body.text(), int)
        mass_body = split_string(self.mass_body.text(), float)
        inertia_body = split_string_multilist(
            self.inertia_body.text(), float, sep=",", sep_multi=";"
        )
        ...
        data_st.append(
            self.__raise_error(
                [
                    el
                    for el in range(1)
                    if os.path.isfile(os.path.join(self._prj, mesh_body))
                ],
                self.mesh_f_t2,
                1,
                self.add_err_t2,
        ...
        data_st.append(
            self.__raise_error(id_body, self.id_body, 1, self.add_err_t2)
        )
        data_st.append(
            self.__raise_error(cog_body, self.cog_body, 3, self.add_err_t2)
        )
        data_st.append(
            self.__raise_error(cs_body, self.cs_body, 3, self.add_err_t2)
        )
        data_st.append(
            self.__raise_error(
                dof_body, self.dof_body, 0, self.add_err_t2, gt=True
            )
        )
        data_st.append(
            self.__raise_error(
                chil_body, self.chil_body, 0, self.add_err_t2, gt=True
            )
        )
        data_st.append(
            self.__raise_error(
                parent_body, self.parent_body, 1, self.add_err_t2
            )
        )
        data_st.append(
            self.__raise_error(mass_body, self.mass_body, 1, self.add_err_t2)
        )
        data_st.append(
            self.__raise_error(
    ...
    def remove_data_tab_body(self, clear_all=False):
        if clear_all:
            ...
            rows = sorted(
                set(index.row() for index in self.tab_body.selectedIndexes())
            )
            for row in rows:
    ...
    def populate_prj(self):
        ...
        self.fre_def.setText(
            ",".join([str(el) for el in in_g["frequency_def"]])
        )
        self.angles_def.setText(
            ",".join(
                [
                    str(el)
                    for iel, el in enumerate(in_g["angle_def"])
                    if iel == 0
                ]
            )
        )
