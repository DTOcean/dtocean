...
class DataStructure:
    ...
    def set_inputs(self):
        ...
        bodies = [
            el
            for el in self.data_dic["inputs_hydrodynamic"]["body_inputs"][
                "body"
            ]
        ]
        n_body = len(bodies)
        ...
        fid = open(gnr, "w")
        fid.write(
            "--- PTO and Mooring definition ---------------------------------------------------------------\n"
        )
        fid.write(
            "{}		! Number of degree of freedom\n".format(
                self.data_dic["inputs_hydrodynamic"]["general_input"]["ndof"][0]
            )
        )
        for pto_i in range(
            len(
                self.data_dic["inputs_hydrodynamic"]["general_input"]["pto_dof"]
            )
        ):
            fid.write(
                "{} ".format(
                    self.data_dic["inputs_hydrodynamic"]["general_input"][
                        "pto_dof"
                    ][pto_i]
                )
            )
        ...
        fid.write(
            "--- frequency and direction to be analysed ---------------------------------------------------\n"
        )
        fid.write(
            "{} {} {} ! number of frequency, min frequency, max frequency [rad/s]\n".format(
                *self.data_dic["inputs_hydrodynamic"]["general_input"][
                    "frequency_def"
                ]
            )
        ...
        fid.write(
            "{} ! Number of wave directions, for simple body 10 is ok for more complex the number should be set to 20 or bigger\n".format(
                self.data_dic["inputs_hydrodynamic"]["general_input"][
                    "angle_def"
                ][0]
            )
        ...
        fid.write(
            "{} {} {} {} {} {}\n".format(
                *self.data_dic["inputs_hydrodynamic"]["body_inputs"][
                    "shared_dof"
                ]
            )
        )
        ...
        for iel, el_k in enumerate(bodies):
            ...
            fid.write(
                "{} {} {}    ! body CoG wer to the point of application\n".format(
                    *el["cog"]
                )
            )
            fid.write(
                "{}    ! parent body ID (-1 if the body is the root)\n".format(
                    el["parent_body"][0]
                )
            )
            fid.write(
                "{}			 	! number of dof between the body and its parent\n".format(
                    ndof_bi
                )
            )
