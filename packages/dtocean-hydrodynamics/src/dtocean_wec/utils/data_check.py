...
def check_inputs_hydrodynamic(hyd_d, prj_path):
    ...
    ndof = cast(int, g_i["ndof"])
    pto_dof = cast(list[int], g_i["pto_dof"])
    ...
    if not np.all(np.array(pto_dof) <= ndof):
        err.append("pto dof greater then the total number of dof")
    if not np.all(np.array(mooring_dof) <= ndof):
        err.append("mooring dof greater then the total number of dof")

    if g_i["water_depth"] == 0 and g_i["get_array_mat"] == 1:
        err.append(
            "the water depth cannot be zero when the array interaction matrixes need to be solved."
        )
    ...
    elif np.any([True for el in b_i["shared_dof"] if el > 1]) or np.any(
        [True for el in b_i["shared_dof"] if el < 0]
    ):
    ...
    if "body" in b_i.keys():
        ...
        if bodies:
            for bd in bodies:
                ...
                err += check_ID(bd["ID"])
                err += check_mesh(
                    bd["mesh"], os.path.join(prj_path, "raw_data")
                )
