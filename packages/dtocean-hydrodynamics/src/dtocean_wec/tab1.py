...
class ReadDb(QWidget, Ui_T1):
    ...
    def load_data(self):
        ...
        description_file = os.path.join(
            self._wec_db_folder,
            current_case,
            "{}_description.html".format(current_case),
        ).replace("\\", "/")
    ...
    def save_project(self):
        current_case = str(self.cb_wec_t1.currentText()).replace(" ", "_")
        data_f = os.path.join(self._wec_db_folder, current_case)

        input_type = 1

        self._data["inputs_hydrodynamic"]["general_input"]["input_type"] = (
            input_type
        )
        self._data["inputs_hydrodynamic"]["general_input"]["data_folder"] = (
            data_f
        )
    ...
    def populate_prj(self):
        ...
        if os.path.isdir(folder):
            for sub in next(os.walk(folder))[1]:
                ...
                if "{}_results.hdf5".format(sub) in os.listdir(
                    sub_a
                ) and "{}_description.html".format(sub) in os.listdir(sub_a):
                    sub_ls.append(sub)
            if not sub_ls:
                print(
                    "Ops, something went wrong. The wec db folder seems not valid anymore. Check the src folder path"
                )

        if not self._init_flag:
            data_folder = self._data["inputs_hydrodynamic"]["general_input"][
                "data_folder"
            ]
            if os.path.isdir(data_folder):
                case = os.path.basename(os.path.normpath(data_folder))
                if "{}_results.hdf5".format(case) in os.listdir(
                    data_folder
                ) and "{}_description.html".format(case) in os.listdir(
                    data_folder
                ):
                    if case in sub_ls:
                        sub_ls.remove(case)
