...
from PySide6.QtWidgets import QFileDialog, QMessageBox, QRadioButton, QWidget
...
class PowerPerformance(QWidget, Ui_Fit):
    ...
    def set_dimensions(self, angle):
        if self._data is None:
            raise RuntimeError("Call set_data first")
    ...
    def fitting(self):
        ...
        per_fit.perf_fitting(
            self._data["inputs_pm_fit"]["machine"],
        ...
        pfit = {
            ...
            "user_power_matrix": self._data["inputs_pm_fit"]["machine"][
                "power_matrix"
            ],
        }
    ...
    def load_data(self):
        ...
        dat1 = read_file(
            os.path.join(
                self.data_folder, "performance_fit_scatter_diagram.csv"
            )
        )
        dat2 = read_file(
            os.path.join(self.data_folder, "performance_fit_power_matrix.csv")
        )
        dat3 = read_file(
            os.path.join(self.data_folder, "performance_fit_pto.csv")
        )
        dat4 = read_file(
            os.path.join(self.data_folder, "performance_fit_mooring.csv")
        )
        dat5 = read_file(
            os.path.join(self.data_folder, "performance_fit_ext_damping.csv")
        )
        dat6 = read_file(
            os.path.join(self.data_folder, "performance_fit_ext_stiffness.csv")
        )
        ...
        if stat:
            ...
            if not self._data["inputs_hydrodynamic"]["general_input"][
                "get_array_mat"
            ]:
                print(
                    "WARNING: The hydrodynamic data has been calculated without the foce transfer matrix option"
                )
                print("Therefore the performance fitting cannot be performed.")
                print(
                    "To proceed further click on the skip performance fitting button."
                )
                self.btn_fitting.setEnabled(False)
            else:
        ...
        if "inputs_pm_fit" in self._data.keys():
            if "data_folder" in self._data["inputs_pm_fit"].keys():
                self.data_folder = str(
                    self._data["inputs_pm_fit"]["data_folder"]
                )
                if not os.path.isdir(self.data_folder):
                    msg = (
                        "The data path stored in the project is not valid "
                        "anymore. The system will attempt to load the data "
                        "saved in the project. Do you want to load the "
                        "data (Yes) or specify another data folder (No)"
                    )
                ...
                if not stat:
                    if "p_fit" in self._data.keys():
                        self.btn_plot_pfit.setEnabled(True)
                        self.set_dimensions(
                            self._data["inputs_pm_fit"]["site"]["wave_angles"]
                            * 180
                            / np.pi
                        )
                        self.trigger_results.emit(self._data["p_fit"])
...
def check_pfit_folder(folder):
    if not os.path.isdir(folder):
        return "Invalid path"
    requirements = [
        "performance_fit_scatter_diagram.csv",
    ...
    for req in requirements:
        if req not in os.listdir(folder):
...
def scatter_diagram_data(data):
    te = np.asarray([float(x) for x in data[4] if x != ""])
    hm0 = np.asarray([float(x) for x in data[6] if x != ""])
    angles = np.asarray([float(x) for x in data[8] if x != ""])
    ...
    for angle in angles:
        ...
        for lines in range(len(hm0)):
            scatter_diagram[:, lines, a_ind] = np.asarray(
                [float(x) for x in data[ind + lines + 1] if x != ""]
            )
...
def power_matrix_data(data, angles, shape):
    ...
    for angle in angles:
        ...
        for lines in range(shape[1]):
            power_matrix[:, lines, a_ind] = np.asarray(
                [float(x) for x in data[ind + lines + 1] if x != ""]
            )
...
def pto_data(data, ndof):
    ...
    for dof in range(ndof):
        pto.append([float(x) for x in data[dof] if x != ""])
...
def mooring_data(data, ndof):
    mooring = []
    for dof in range(ndof):
        mooring.append([float(x) for x in data[dof] if x != ""])
...
def ext_damping_data(data, ndof):
    ext_d = []
    for dof in range(ndof):
        ext_d.append([float(x) for x in data[dof] if x != ""])
...
def ext_stiffness_data(data, ndof):
    ext_k = []
    for dof in range(ndof):
        ext_k.append([float(x) for x in data[dof] if x != ""])
...
def check_data_sanity(ndof, machine_spec, site_spec):
    ...
    if not p2.shape == (ndof, ndof):
        error_s.append(
            "The mooring stiffness shape does not match the number of dof"
        )
        error_s.append("pto damping shape: {} - dofs {}".format(p2.shape, ndof))

    if not p4.shape == (ndof, ndof):
        error_s.append(
            "The external damping shape does not match the number of dof"
        )
        error_s.append("pto damping shape: {} - dofs {}".format(p4.shape, ndof))
    if not p5.shape == (ndof, ndof):
        error_s.append(
            "The external stiffness shape does not match the number of dof"
        )
        error_s.append("pto damping shape: {} - dofs {}".format(p5.shape, ndof))

    if not p3.shape == p9.shape:
        error_s.append(
            "The scatter diagram shape does not match the power matrix one"
        )
        error_s.append(
            "Scatter diagram shape: {} - Power matrix shape {}".format(
                p9.shape, p3.shape
            )
        )
    ...
    if not p9.shape == (len(p6), len(p7), len(p8)):
        ...
        error_s.append(
            "Scatter diagram shape: {} - axis shape {}".format(
                p9.shape, (len(p6), len(p7), len(p8))
            )
        )

    elif not np.allclose(p9.sum(), 1.0, atol=1e-1):
        error_s.append(
            "The probability of occurence of the different sea state does not sum to 1."
        )
