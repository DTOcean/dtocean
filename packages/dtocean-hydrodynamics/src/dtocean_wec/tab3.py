...
class ReadNemoh(QWidget, Ui_T3):
    ...
    def browse_data(self):
        folder = QFileDialog.getExistingDirectory(
            self, "Select the folder that contains the Nemoh results and inputs"
        )
    ...
    def check_body_data(self):
        ...
        if not os.path.isfile(mesh_body):
            mesh_body = os.path.join(
                self._prj, self._raw, os.path.basename(mesh_body)
            )
        ...
        dof_body = split_string_multilist(
            self.dof_body_t3.text(), sep=",", sep_multi=";"
        )
        chil_body = split_string_multilist(
            self.chil_body_t3.text(), sep=",", sep_multi=";"
        )
        ...
        inertia_body = split_string_multilist(
            self.inertia_body_t3.text(), float, sep=",", sep_multi=";"
        )
        ...
        data_st.append(
            raise_error(
                [el for el in range(1) if os.path.isfile(mesh_body)],
                self.mesh_f_t3,
                1,
                self.add_err_t3,
            )
        ...
        data_st.append(
            raise_error(id_body, self.id_body_t3, 1, self.add_err_t3)
        )
        data_st.append(
            raise_error(cog_body, self.cog_body_t3, 3, self.add_err_t3)
        )
        data_st.append(
            raise_error(cs_body, self.cs_body_t3, 3, self.add_err_t3)
        )
        data_st.append(
            raise_error(dof_body, self.dof_body_t3, 0, self.add_err_t3, gt=True)
        )
        data_st.append(
            raise_error(
                chil_body, self.chil_body_t3, 0, self.add_err_t3, gt=True
            )
        )
        data_st.append(
            raise_error(parent_body, self.parent_body_t3, 1, self.add_err_t3)
        )
        data_st.append(
            raise_error(mass_body, self.mass_body_t3, 1, self.add_err_t3)
        )
        data_st.append(
            raise_error(
                [item for sublist in inertia_body for item in sublist],
                self.inertia_body_t3,
                9,
                self.add_err_t3,
            )
        )
    ...
    def add_data_tab_body(self):
        ...
        if -1 in data_st:
            ...
            self.tab_body_t3.setItem(
                row_position,
                0,
                QTableWidgetItem("".join([str(x) for x in data[0]])),
            )
            self.tab_body_t3.setItem(
                row_position,
                1,
                QTableWidgetItem("".join([str(x) for x in data[1]])),
            )
            self.tab_body_t3.setItem(
                row_position,
                2,
            ...
            self.tab_body_t3.setItem(
                row_position, 3, QTableWidgetItem(fn)
            )  # data[3]
            self.tab_body_t3.setItem(
                row_position,
                4,
                QTableWidgetItem(",".join([str(x) for x in data[4]])),
            )
            self.tab_body_t3.setItem(
                row_position,
                5,
                QTableWidgetItem(",".join([str(x) for x in data[5]])),
            )
            self.tab_body_t3.setItem(
                row_position,
                6,
                QTableWidgetItem(
                    ";".join([str(x) for x in data[6]])
                    .replace("[", "")
                    .replace("]", "")
                ),
            )
            ...
            self.tab_body_t3.setItem(
                row_position,
                8,
                QTableWidgetItem("".join([str(x) for x in data[8]])),
            )
    ...
    def remove_data_tab_body(self, clear_all=False):
        if clear_all:
            ...
            rows = sorted(
                set(index.row() for index in self.tab_body_t3.selectedIndexes())
            )
            for row in rows:
    ...
    def browse_mesh(self):
        folder, _ = QFileDialog.getOpenFileName(
            self, "Select the mesh file of the body"
        )
    ...
    def submit_input_data(self):
        ...
        if status[0]:
            ...
            self._data["inputs_hydrodynamic"]["general_input"]["ndof"] = (
                split_string(self.ndof_t3.text(), int)
            )
            self._data["inputs_hydrodynamic"]["general_input"]["pto_dof"] = (
                split_string(self.pto_dof_t3.text(), int)
            )
            self._data["inputs_hydrodynamic"]["general_input"][
                "mooring_dof"
            ] = split_string(self.moor_dof_t3.text(), int)
            ...
            self._data["inputs_hydrodynamic"]["general_input"][
                "get_array_mat"
            ] = int(self.cb_gen_array_mat_t3.isChecked())
    ...
    def pull_data_from_form(self):
        def get_item(x, y):
            item = self.tab_body_t3.item(x, y)
            if item is None:
        ...
        for el in range(self.tab_body_t3.rowCount()):
            ...
            bodies[str_bi] = {
                ...
                "mesh": mesh_body,
                "cog": cog_body,
                "child_dof_position": chil_body,
                "dof_with_parent": dof_body,
                "number_child": 1,
                "axis_angles": cs_body,
    ...
    def populate_prj(self):
        ...
        if bodies == -1:
            ...
            for bi in bodies:
                ...
                self.cog_body_t3.setText(
                    ",".join([str(el) for el in di["cog"]])
                )
                self.cs_body_t3.setText(
                    ",".join([str(el) for el in di["axis_angles"]])
                )
                self.parent_body_t3.setText(
                    ",".join([str(el) for el in di["parent_body"]])
                )
                self.mass_body_t3.setText(
                    ",".join([str(el) for el in di["mass"]])
                )
                ...
                self.inertia_body_t3.setText(
                    ";".join([str(el) for el in inertia_ls])
                    .replace("[", "")
                    .replace("]", "")
                )
