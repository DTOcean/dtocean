...
class SearchOptimum(object):
    ...
    def __set_problem_parameters(self):
        ...
        if self._Opt == 1:
            if self._Val == "rectangular":
                ...
                err_str = (
                    "Error[UserArray input]: For the Option 1 the "
    ...
    def eval_optimal_layout(self):
        ...
        if xopt == -1:
            errStr = (
                "Error[OptimisationResults]: No array configuration "
        ...
        if self._Opt == 1:
            ...
            self._array.coord = (
                self._Val * int(xmap[0]) / self._normalisation_point
            )
        ...
        while True:
            ...
            if inside.any() and not self._array.minDist_constraint:
                if self._array.coord[inside].shape[0] > self._max_num_dev:
                    ...
                    return remap(
                        self._Val, self._normalisation_point, IC, IR, beta, psi
                    )
    ...
    def optimCostFun(self, x):
        ...
        if beta < 0.05 and self._Opt != 3:
            module_logger.debug(
                "The angle between rows and " "columns is too close to zero."
            )
        ...
        if self._Opt == 3:
            ...
            module_logger.info(
                "Layout parameters: IC: {} IR: {} beta: {} " "psi: {}".format(
                    IC, IR, beta, psi
                )
            )
        ...
        if inside.any() and not self._array.minDist_constraint:
            ...
            if n_devs > self._max_num_dev:
                msg_str = (
                    "Layout not valid: {} devices generated, "
                    "but maximum is {}"
                ).format(n_devs, self._max_num_dev)
            ...
            if res.q_array >= self._min_q_factor:
                if self._debug:
                    module_logger.info(
                        "Valid config: actual q-factor {} --> "
                        "min q-factor {}".format(
                            res.q_array, self._min_q_factor
                        )
                    )
        ...
        if self._debug:
            module_logger.warning(
                "WARNING! For the given configuration "
...
def method_brutal_force(searcher, N=5):
    ...
    index = fit.argmin()
    pickle.dump(
        [fit, x, y], open("optimisation_results_brutal_force.pkl", "wb")
    )
...
def method_cma_es(searcher, tolfun=1e1, tolx=1e-3, maxiter=200, maxfevals=2000):
    """
    ...
    if not x0:
        warning_str = (
            "Could not find a suitable starting point "
            "for the optimiser, the centroid of the "
            "parameter space is used instead"
        )
        module_logger.warning(warning_str)
        x0 = searcher.opt_dim * [
            (searcher._min_bound + searcher._max_bound) / 2.0
        ]

    es = cma.CMAEvolutionStrategy(
        x0,
        2,
        {"bounds": [searcher._min_bound, searcher._max_bound], "verb_disp": 0},
    )
