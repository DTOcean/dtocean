...
def get_unfeasible_regions(
    ...
    if stat == -1:
        errStr = (
            "Error[InstalDepth]: No feasible installation area "
            "has been found for the device for the given bathymetry."
        )
        raise ValueError(errStr)
    elif stat == 1:
        module_logger.debug(
            "No NOGO areas related to the machine depth "
    ...
    unfeasible_mask = np.logical_not(
        (safe_Z >= z_bounds[0]) * (safe_Z <= z_bounds[1])
    )
...
def clustering(im, pixel_area, area_thr=10, g_fil=0.5, debug=False):
    ...
    areas = (
        ndimage.sum(im, label_im, range(nb_labels + 1)) * pixel_area
    )  # cos each pixels has an area of dx*dy
...
def check_bathymetry_format(xyz, bound):
    ...
    if not isinstance(xyz, np.ndarray):
        raise IOError(
            "The data type of the bathymetry needs to be a numpy.ndarray"
        )
    if len(xyz.shape) == 1:
        nx, ny = 1, len(xyz)
    ...
    if not (nx == 3 or ny == 3):
        raise IOError(
            "The size of the bathymetry needs to be [nx3], where n is the number of datapoints"
        )
