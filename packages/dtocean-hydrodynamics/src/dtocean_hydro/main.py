...
class WP2:
    ...
    def __init__(
        ...
        self.iArray = Array_pkg(
            WP2input.S_data.LeaseArea,
        ...
        if not WP2input.internalOptim:
            module_logger.info(
                "The user provided an external map of the "
            ...
            if WP2input.M_data.tidalFlag:
                ...
                self.iHydro = Hydro_pkg(
                    devType, V, U, p, TI, x, y, SSH, bathy, beta, alpha
                )

            else:
                devType = "W"
                freqs = read_wec_sol.read_freq(WP2input.M_data.wave_data_folder)
                cfreqs = 2.0 * np.pi * freqs
                ...
                wdirs = set_wdirs_multibody(
                    B.copy(), specType[-1], mA, yA, debug=debug
                )
            ...
            if WP2input.M_data.tidalFlag:
                ...
                if WP2input.M_data.tidal_data_folder is None:
                    ...
                    self.cfd_data = read_database(path_dict["tidal_share_path"])
                else:
                ...
                if min(self.iArray.Dmin) / 2.0 < self.iWEC.radius:
                    Dmin_new = ()
                    for D in self.iArray.Dmin:
                        ...
                        module_logger.info(
                            "Interaction theory min requirement: " "{}".format(
                                self.iWEC.radius * 2
                            )
                        )
                        module_logger.info(
    ...
    def optimiseExternalTab(self):
        ...
        if np.any(mask_q):
            ...
            return WP2output(
                OptConf["Annual_Energy_Production_Array"],
            ...
            raise ValueError(
                "No feasible configurations found in the given " "result table"
            )
    ...
    def optimisationLoop(self):
        ...
        module_logger.info(
            "Finishing the WP2 task: Evaluation of the "
        ...
        if not np.any(inside):
            msgStr = (
                "All devices have been excluded. Check lease area "
        ...
        if Opt == 2 and not inside.all():
            ...
            err_msg = (
                "Devices at positions {} have been excluded. Check "
        ...
        if not self.iInput.M_data.tidalFlag:
            ...
            dev_depth = get_device_depths(self.iInput.S_data.Bathymetry, layout)
            device_average_depth = -np.mean(dev_depth) + SSH

            if np.any(
                np.logical_or(
                    bem_depth <= site_depth * 0.99,
                    bem_depth >= site_depth * 1.01,
                )
            ) or np.any(
                np.logical_or(
                    bem_depth <= device_average_depth * 0.99,
                    bem_depth >= device_average_depth * 1.01,
                )
            ):
                module_logger.warning(
                    warning_str.format(
                        bem_depth, site_depth, device_average_depth, site_depth
                    )
                )
        ...
        norm_dir = self.iInput.S_data.Main_Direction / np.linalg.norm(
            self.iInput.S_data.Main_Direction
        )
    ...
    def _get_hyd_obj(self):
        ...
        if self.iInput.M_data.tidalFlag:
            ...
            hyd_obj = MultiBody(self.iHydroMB, self.iWEC, cylamplitude=True)
    ...
    def _get_optim_obj(self, hyd_obj):
        if self._optim_method == 1:
            ...
            raise IOError(
                "The specified optimisation method ID is out of " "range."
            )

        opt_obj = self._search_class(
            opt_func,
            hyd_obj,
            self.iArray,
...
def get_device_depths(bathymetry, layout):
    ...
    if layout.shape[1] != 2:
        raise IndexError(
            "Layout array second dimension does not have two " "elements"
        )
