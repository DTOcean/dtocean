...
class Array_pkg(object):
    ...
    def generator(self, NR, NC, IR, IC, beta, psi):
        ...
        if not self.minDist_constraint:
            ...
            rot_angle = self.mainAngle - np.pi / 2.0
    ...
    def checkMinDist(self):
        ...
        grid_inside_percent = get_grid_inside_ellipse_percent(
            self.coord, angle, self.Dmin[0], self.Dmin[1]
        )
    ...
    def checkout(self, nogo_list=None, mindist_raise=False):
        ...
        if self.minDist_constraint:
            ...
            if self._mindist_percent_max is not None:
                msg_str += (". Maximum ellipse transect percentage: {}").format(
                    self._mindist_percent_max
                )
            ...
            if np.any(lease_mask):
                ...
                if self.Nogo_bathymetry is not None:
    ...
    def inner_region(self, points):
        ...
        for inp, point in enumerate(points[1:]):
            nodedistance[inp] = np.sqrt(
                (
                    (point[0] - points[0][0]) ** 2
                    + (point[1] - points[0][1]) ** 2
                )
            )
        nodedistance[-1] = np.sqrt(
            (
    ...
    def show(self, inside=None, ax=None):
        ...
        Leasep_unbuffered = np.zeros(
            (len2(self._lease_unbuffered) + 1, 2), dtype=float
        )
        ...
        ax.add_patch(
            patches.Arrow(
                self.centroidLease[0]
                - scale * np.cos(self.mainAngle - 2 * np.pi),
                self.centroidLease[1]
                - scale * np.sin(self.mainAngle - 2 * np.pi),
