...
class WP2input:
    ...
    def checkInput(self):
        ...
        if self.S_data.NogoAreas is not None:
        ...
        if self.S_data.Main_Direction is not None:
        ...
        if self.M_data.tidalFlag:
            if not isinstance(
                self.S_data.MeteoceanConditions["SSH"], np.ndarray
            ):
                status -= 1
                errStr.append(
                    "The SSH value should be numpy.ndarray for "
                    "tidal devices."
                )
        ...
        if (
            ...
            if not os.path.isfile(fname):
                status -= 1
                errStr.append(
                    "The wec_solution.h5 file is missing!\n"
        ...
        if self.M_data.InstalDepth is not None:
            if not isinstance(
                self.M_data.InstalDepth, (list, np.ndarray, tuple)
            ):
                status -= 1
                errStr.append(
                ...
                if len(self.M_data.InstalDepth) == 1:
                    status -= 1
                    errStr.append(
                        "The installation depth requires two "
                    ...
                    if self.M_data.InstalDepth[0] == self.M_data.InstalDepth[1]:
                        status -= 1
                        errStr.append(
                            "The two installation depth values need "
                            "to be dinstinct."
                        )
                else:
                    status -= 1
        ...
        if Opt == 2:
            ...
            if not Value.shape[1] == 2:
                if Value.shape[0] == 2:
                    ...
                    raise ValueError(
                        "The fixed array layout need to have",
                        "a shape of nbodyx2.",
        ...
        if self.M_data.tidalFlag:
            if self.M_data.lCS[-1] < self.M_data.Clen[0] / 2:
                raise ValueError(
                    "The turbine radius is larger than the ",
    ...
    def irrotational_flow(self):
        ...
        if not len(u_sh) == 3:
            raise IOError(
                "The shape of the velocity field is incorrect",
        ...
        for el in range(u_sh[2]):
            ...
            if np.sqrt(curl.max() ** 2 + curl.min() ** 2) > 0.1:
                raise IOError(
                    "The input velocity field is highly rotational ",
    ...
    def getInstallationAreaConstraints(self):
        ...
        if len(Bathymetry) == 1 and (
            ...
            self.S_data.NogoAreas_bathymetry = (
                None  # the whole lease area can be used
            )
            if self.M_data.tidalFlag:
                X, Y = np.meshgrid(
                    self.S_data.MeteoceanConditions["x"],
                    self.S_data.MeteoceanConditions["y"],
                )
                Z = X * 0.0 + Bathymetry  # generate a flat bathymetry
        ...
        if not self.M_data.tidalFlag:
            ...
            mask_outofwater = (
                Bathymetry[:, -1] <= 0
            )  # true all points below swl
            mask_nan = np.logical_not(
                np.isnan(Bathymetry[:, -1])
            ...
            if self.S_data.NogoAreas:
                if not isinstance(self.S_data.NogoAreas, list):
                    raise IOError(
                        "The nogo areas input by the user needs to "
            ...
            if Bathymetry[bathymetry_mask, -1].size == 0:
                raise ValueError(
                    "No valid points found within lease area. "
    ...
    def getMainAngle(self):
        ...
        if self.S_data.Main_Direction is None:
            ...
            if self.M_data.tidalFlag:
                ...
                ind_max_direction = np.argmax(
                    self.S_data.MeteoceanConditions["p"]
                )
                U = np.nanmean(
                    self.S_data.MeteoceanConditions["U"][
                        :, :, ind_max_direction
                    ]
                )
                V = np.nanmean(
                    self.S_data.MeteoceanConditions["V"][
                        :, :, ind_max_direction
                    ]
                )
                ...
                ind_max_direction = np.argmax(
                    np.sum(self.S_data.MeteoceanConditions["p"], (0, 1))
                )
                angle = self.S_data.MeteoceanConditions["B"][ind_max_direction]
                self.S_data.Main_Direction = np.array(
                    [np.cos(angle), np.sin(angle)], dtype=float
                )

        self.S_data.mainAngle = anglewrap(
            np.arctan2(
                self.S_data.Main_Direction[1],
    ...
    def compress_lease_area(self):
        ...
        overall_buffer = max(
            MCT_buffer, abs(max(0, self.S_data.boundary_padding))
        )
    ...
    def rated_power_consistency_check(self):
        ...
        if self.M_data.tidalFlag:
            ...
            (
                ...
                scatter_diagram,
                power_matrix,
            ) = reader.read_performancefit_solution(
