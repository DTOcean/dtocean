...
class WakeInteraction:
    ...
    def solve_flow(self, debug=False):
        ...
        while ind_err > self._criterior and loop_counter < self._max_loop:
            (newVel, newSpeed, newTI, newTKE) = _solve_flow(
                self._turbine_count,
        ...
        if debug:
            ...
            module_logger.info(
                "...done after " + str(end - start) + " seconds."
            )

        if ind_err <= self._criterior:
            log_msg = (
                "Device interaction solver converged in {} " "iteration(s)"
            ).format(loop_counter)
            module_logger.debug(log_msg)

        else:
            log_msg = (
                "Device interaction solver failed to converge after "
                "the maximum {} iterations. Residual at last "
                "interation was {}"
            ).format(self._max_loop, ind_err)
    ...
    def solv_wake(self, debug=False, debug_plot=False):
        ...
        for i in range(self._turbine_count):
            turb = "turbine{:0{width}d}".format(i, width=n_digits)
            self._wakeShape[turb] = WakeShape(
    ...
    for i in range(turbine_count):
        ...
        for j in range(turbine_count):
            if j in turb_distances[turb].keys():
                (wake_mat[i, j], tke_mat[i, j]) = turb_wakes[
                    turb
                ].get_velocity_TKE(
                ...
                wake_mat[i, j] = np.sqrt(
                    turb_velocity[0, i] ** 2 + turb_velocity[1, i] ** 2
                )
                tke_mat[i, j] = np.nan

    superposition_model = DominantWake(turb_velocity, wake_mat)
    coefficients = superposition_model.coefficients

    TKE_coeff_mat = get_wake_coefficients(turb_TKE, tke_mat)
