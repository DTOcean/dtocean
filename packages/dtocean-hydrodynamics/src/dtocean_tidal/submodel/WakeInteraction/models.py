...
class DominantWake(object):
    ...
    def __init__(self, turb_velocity, wake_matrix):
        turb_speed = np.sqrt(
            turb_velocity[0, :] ** 2 + turb_velocity[1, :] ** 2
        )
    ...
    def get_dominant(self, matrix):
        ...
        dominant = matrix[self.indexes, range(self._coefficients.shape[1])]

        return dominant


def get_wake_coefficients(baseline, interaction_matrix):
