...
class Wake:
    ...
    def __init__(
        self, dataframe, U_dict, V_dict, TKE_dict, turbParams, BR, debug=False
    ):
        self._debug = debug
        ...
        if BR < 0.0:
            ...
            self.BR = 0.7
            module_logger.debug(
                "Blockage ratio > 70%... reaching model's " "theoretical limit."
            )
        else:
            self.BR = BR
        ...
        if ry < 0.0:
            ...
            module_logger.debug(
                "Relative angle with inflow > 90.0 deg... "
    ...
    def read_at_point(self, x, y, ct, ti, debug=False):
        ...
        if debug:
            module_logger.info("Picking CFD datasets...")
        ...
        if ctbounds == []:
            module_logger.debug(
                (
                    "Requested Ct value {} is not covered by the "
                    "current CFD database."
                ).format(ct)
            )
            ...
            module_logger.debug(
                ("Ct value of {} will be " "used.").format(self._cts[idx])
            )

        if tibounds == []:
            module_logger.debug(
                (
                    "Requested TI value {} is not covered by "
                    "the current CFD database."
                ).format(ti)
            )
            ...
            module_logger.debug(
                ("TI value of {} will be " "used.").format(self._tis[0])
            )
        ...
        wght[0] = (btmp[1] - btmp[2]) * (ct - atmp[2]) + (atmp[2] - atmp[1]) * (
            ti - btmp[2]
        )
        wght[1] = (btmp[2] - btmp[0]) * (ct - atmp[2]) + (atmp[0] - atmp[2]) * (
            ti - btmp[2]
        )
        wght[0:1] = wght[0:1] / (
            (btmp[1] - btmp[2]) * (atmp[0] - atmp[2])
        ...
        if debug:
            module_logger.info("...interpolation...")
        u = 0.0
        v = 0.0
        tke = 0.0
        ...
        for n in range(3):
            ...
            tkemat = self._dfTKE["ti" + str(tmpTi)]["ct" + str(tmpCt)] * wght[n]
    ...
    def get_velocity_TKE(self, distance, velHub, tiHub, debug=False):
        ...
        try:
            ...
            if debug:
                module_logger.info(
                    "Flow speed value outside of Ct's "
        ...
        if tiHub < self._tis[0]:
            ...
            module_logger.debug(
                (
                    "T.I. < {}... reaching " "parametric database's " "limit."
                ).format(self._tis[0])
            )
        ...
        if (
            (Ct > np.min(self._cts) and ry < np.radians(89.0))
            and (X.min() < x < X.max())
            and (Y.min() < y < Y.max())
        ):
    ...
    def __init__(
        ...
        if self._bounding_box.contains(pt1) or self._bounding_box.contains(pt2):
            if self._bounding_box.contains(pt1):
                Lref = line(
                    [self.streambot[0][-2], self.streambot[1][-2]],
                    [self.streambot[0][-1], self.streambot[1][-1]],
                )
                Lbis = line(
                    [self.streamtop[0][-2], self.streamtop[1][-2]],
