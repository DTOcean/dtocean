...
class CallTidal:
    ...
    def __init__(
        self, WP2Hydro, WP2Input, cfd_data, debug=False, debug_plot=False
    ):
        ...
        self.coord: Optional[npt.NDArray] = None

        # machine features
        Cp = [
            WP2Input.M_data.tidal_velocity_curve,
    ...
    def energy(self, coord):
        ...
        if self.__mct_flag:
            (
                ...
                pow_perf_dev_state, pow_perf_dev_state_ni, ti_dev_state, turb
            )
    ...
    def __output_tidal(self):
        ...
        q_ar = np.sum(self.power_prod_array) / np.sum(
            self.power_prod_array_no_int
        )
        q_dev = np.sum(self.power_prod_perD_perS, 1) / np.sum(
            self.power_prod_perD_perS_ni, 1
        )
    ...
    def __set_turbine_and_features(self):
        ...
        if self.coord is None:
            raise RuntimeError(
                "Turbine positions are not set. Call the energy method first."
            )
    ...
    def __set_coordinates(self, coord):
        ...
        if self.__mct_flag:
            self.coord = _get_perp_positions(
                self.__base_feature["OA"], self.__mct_dist, coord
            )
        else:
            self.coord = coord
...
def _map_mct_results(
    ...
    pow_perf_dev_state_ni: npt.NDArray,
    ti_dev_state: list[dict[str, Any]],
    turbines: dict[str, Any],
):
    """
    ...
    nb //= 2
    ti_dev_state_n: list[dict[str, Any]] = []
    turbines_n: dict[str, dict[str, Any]] = {}

    n_digits_old = len(str(nb * 2))
    n_digits_new = len(str(nb))

    for kk in range(nb):
        left_turb_name = "turbine{:0{width}d}".format(kk, width=n_digits_old)
        right_turb_name = "turbine{:0{width}d}".format(
            kk + nb, width=n_digits_old
        )
        turb_name = "turbine{:0{width}d}".format(kk, width=n_digits_new)

        pow_perf_dev_state[kk,] = (
            pow_perf_dev_state[kk,] + pow_perf_dev_state[kk + nb,]
        )
        pow_perf_dev_state_ni[kk,] = (
            pow_perf_dev_state_ni[kk,] + pow_perf_dev_state_ni[kk + nb,]
        )

        turb_pos = 0.5 * (
            turbines[left_turb_name]["position"]
    ...
    for ll in range(n):
        ...
        for kk in range(nb):
            ...
            turb_ti = 0.5 * (
                ti_dev_state[ll][left_turb_name]
...
def _get_perp_positions(main_bearing, separation_len, centre_coord):
    ...
    turb_coords = np.vstack(
        (centre_coord + separation_vector, centre_coord - separation_vector)
    )
