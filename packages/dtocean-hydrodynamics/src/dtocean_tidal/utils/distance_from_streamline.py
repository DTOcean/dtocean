...
def distance_from_streamline(
    streamline, turbine_positions, debug=False, debug_plot=True
):
    """
    ...
    """
    if debug:
        module_logger.info("Computing relative distance from hub...")
    ...
    for i in range(n_turbines):
        ...
        # streamline
        if np.isclose(np.sum(turbine_vector[0, :]), 0.0):
            continue
        ...
        perpendicular_x = (
            valid_startx
            - turbine_positions[i, 0]
            + segment_weights * valid_segments[:, 0]
        )
        perpendicular_y = (
            valid_starty
            - turbine_positions[i, 1]
        ...
        turbine_distances[i] = np.array([streamline_distance, min_dist])

    if debug:
        module_logger.info(turbine_distances)
