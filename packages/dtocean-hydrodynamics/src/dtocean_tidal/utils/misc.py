...
def closest_point(pt, pts, debug=False):
    ...
    """
    if debug:
        module_logger.info("Computing closest point index...")
    ...
    if debug:
        module_logger.info("index: {}".format(closest_point))
    if debug:
        module_logger.info("distance: {}".format(dist))
...
def transec_surf(hydro, array, debug=False):
    ...
    if debug:
        module_logger.info("Computing relative blockage ratio RBR...")
    ...
    turb_keys = []
    ref = np.arange(Nturb).astype(int)
    ...
    first_turb_name = "turbine{:0{width}d}".format(first_row[0], width=n_digits)
    last_turb_name = "turbine{:0{width}d}".format(first_row[-1], width=n_digits)
