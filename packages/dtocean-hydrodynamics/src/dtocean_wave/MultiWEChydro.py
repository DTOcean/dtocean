...
class MultiBody(object):
    ...
    def __init__(self, ihydro, iwec, cylamplitude=True, debug=False):
        ...
        self.dir = ihydro.dir
        self.iwec = iwec
        (self.cpto, self.cfit, self.kmooring, self.kfit) = (
            iwec.matrix_zoh_interp(ihydro)
        )
    ...
    def energy(self, coord):
        ...
        Nm = array(
            range(-self.iwec.order.max(), self.iwec.order.max() + 1),
            dtype=float,
        )
        ...
        for orient, thetas in zip(orients, theta_groups):
            ...
            Fex_iso = zeros(
                (self.Fex.shape[:2] + (self.iwec.Fex.shape[2],)),
                dtype=np.complex64,
            )
    ...
    def Hydrodynamics(self, iWEC, direction, D_rot, G_rot, AR_rot):
        ...
        TolCheck = 1e-3
        requires = [
            abs(self.period - iWEC.period) < TolCheck,
        ...
        (TS, TR, DS_array, DR_array, GS_array, GR_array, MS_inter, MR_inter) = (
            self.Interaction(k0, D_rot, G_rot, iWEC.truncorder)
        )

        # Wave scattering between bodies
        self.Scattering(
            k0, direction, TS, DS_array, GS_array, MS_inter, iWEC.order[0]
        )
    ...
    def Interaction(self, k0, D, G, TruncOrder):
        ...
        row = transpose(col)
        Dx = transpose(self.coord[:, 0] * ones((Nb, Nb))) - self.coord[
            :, 0
        ] * ones((Nb, Nb))
        Dy = transpose(self.coord[:, 1] * ones((Nb, Nb))) - self.coord[
            :, 1
        ] * ones((Nb, Nb))
        ...
        matk0, mode, matL = meshgrid(
            k0, array(range(dim)), L, indexing="ij"
        )  # k0 first index, modes are rows and L are columns
        ...
        indpp = (
            col - transpose(col)
        )  # to prevent dividing by zero in si, which used to be defined si= (ind/abs(ind))**ind
        indpp[indpp == 0] = 1
        ...
        for i in range(
            ...
            T[:, col:col_end, row:row_end] = Tij[:, :, :] * exp(
                ind * pi * 1j
            )  # Tji
        ...
        for i in range(Nb):
            ...
            D_array[:, start_dim:end_dim, start_dim:end_dim] = D[
                :, :, :
            ]  # diffraction transfer matrix for the isolated device
        ...
        for i in range(length_of_k0):
            ...
            rowcolR = (
                (
                    array([range(dimp[i, 1])] * Nb).transpose()
                    + array(range(0, dim * Nb, dim))
                )
                .transpose()
                .reshape(-1)
        ...
        for i in range(length_of_k0):
            ...
            ms = solve(
                eye(Nb * dimp[i, 0]) - dot(TS[i], DS_array[i]),
                eye(Nb * dimp[i, 0]),
            )

            mr = solve(
                eye(Nb * dimp[i, 1]) - dot(TR[i], DR_array[i]),
                eye(Nb * dimp[i, 1]),
            )
    ...
    def Scattering(
        self, k0, direction, T, D_array, G_array, M_interaction, BaseOrder
    ):
        """
        ...
        AP = exp(
            -1j
            * (
                matk0 * (X * cos(matdir) + Y * sin(matdir))
                + mode * (pi / 2 + matdir)
            )
        )
        ...
        Fex = zeros(
            (len2(k0), len2(direction), len2(G_array[0][0])), dtype=np.complex64
        )
        ...
        for i in range(len2(k0)):
            ...
            col = (
                ...
                + array(range(0, dim * Nb, dim))
            ).transpose().reshape(-1) + (Nm - Nmi)
            aS = dot(
                dot(AP[i][:, col], D_array[i]), M_interaction[i]
    ...
    def Radiation(
        ...
        for k in range(len2(k0)):
            ...
            AR = dot(
                AR_iso[k][:, colAR], T_re
            )  # each row of aR is constant dof.
            ...
            Ones = eye(Nb, dtype=int).repeat(
                dof, axis=1
            )  # will be used to distribute Crad_t over eye() type
            Frad_k = (
                -(
                    -(freq[k] ** 2) * Ones * Madd_t
                    + 1j * freq[k] * Ones * Crad_t
                )
                + Fex_rad
            )
    ...
    def EnergyBalance(self, power_prod_perD_perS):
        ...
        Spec_ = wave_spec(1.0 / self.period, 1.0, 1.0)
            ...
            for i1 in range(NHs):
                for i2 in range(NTp):
                    ...
                    eP[ind_SS] = (
                        ...
                        * self.Tp[i2]
                        * np.sqrt(
                            (max(self.coord[:, 0]) - min(self.coord[:, 0])) ** 2
                            + (max(self.coord[:, 1]) - min(self.coord[:, 1]))
                            ** 2
                        )
                    )
                    e[ind_SS] = (
                        eP[ind_SS] - power_prod_perD_perS[:, ind_SS].sum()
                    )
