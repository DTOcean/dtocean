...
import logging
import os
from math import cos, log10, pi, tan, tanh
...
def Newton(func, const, x=1, feat=(100, 1e-6, 1e-6)):
    ...
    while found == 0 and k < kmax:
        ...
        if abs(df) < Tol_f * 1e-3:
            ...
            bl = (
                0,
                "Local minimum. log|f| = {:.2f}".format(log10(abs(f) + 1e-99)),
            )
        else:
            " Update x "
            x0 = x
            ...
            if abs(x - x0) < Tol_x and abs(f) < Tol_f:
                ...
                bl = (
                    1,
                    "Converged in {} iterations and with log|f| = {:.2f}".format(
                        k, log10(abs(f) + 1e-99)
                    ),
                )
    " Did not converge "
    if found == 0:
        bl = (
            0,
            "Did not converge. log|f| = {:.2f}".format(log10(abs(f) + 1e-99)),
        )
    return x, bl
...
def WNumber(period, depth):
    ...
    try:
        L = array(
            [Newton(Dispersion_T, (p, depth))[0] for p in period], dtype=float
        )
    except TypeError:
...
def len2(x):
    ...
    try:
        ...
        raise TypeError(
            "Attempted len2(x) but type(x) = {} is not supported in this version".format(
                type(x)
            )
        )
        ...
        for i in range(ax[0]):
            for ii in range(ax[1]):
                for iii in range(ax[2]):
                    s = [
                        format(variable[i, ii, iii, iv], "e")
                        for iv in range(ax[3])
                    ]
                    s = " ".join(s)
...
def CylWaveField(
    X, Y, Z, amplitudes, freq, wnumber, wdepth, Bodiescoord, disregard, plane=0
):
    """
    unused TBD
    """
    ...
    for i in range(NumBodies):
        ...
        for m in mode[Nm:]:
            ...
            if plane == 1:
                ...
                if m > 0:
                    Phii[i] += (
                        a[Nm - m] * (-1) ** m * Psii * exp(1j * -m * thetai)
                    )
    Phii *= 1j * g / freq
    if len2(Z) == 1:
    ...
    for n in [0, 1]:
        ...
        while any(ok.reshape(-1) == 1) and abs(ite) < itemax:
            ...
            for i in range(dims):
                ...
                ok[i, (ok[i] == 1) * (A[binslist] == A[binslist])] = (
                    0  # nan == nan --> False
                )
