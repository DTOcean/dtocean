...
def create_empty_project(folder, prj_name):
    ...
    bodies = body
    body_inputs = {
        "shared_dof": [0, 0, 0, 0, 0, 0],
        "local_cs": [],
        "body": bodies,
    }
    filename = os.path.join(
        folder, "".join([prj_name, "_data_collection.hdf5"])
    )
...
def load_key_from_hdf5(filename, key_str):
    with h5py.File(filename, "r") as h5file:
        data = np.asarray(h5file[key_str])
    return data


def load_dict_from_hdf5(filename, decode_bytes=True):
    with h5py.File(filename, "r") as h5file:
...
def recursively_load_dict_contents_from_group(h5file, path, decode_bytes=True):
    ans = {}
