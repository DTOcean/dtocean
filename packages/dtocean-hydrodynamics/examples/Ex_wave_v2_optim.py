...
" Lease Area "
leaseAreaVertexUTM = (
    np.array([[0, 0], [1000.0, 0], [1000.0, 300.0], [0, 300.0]], dtype=float)
    - 1.0
)
...
FixedArrayLayout = np.array([[0, 100.0], [100, 100.0], [200, 100.0]], "f")
Nogoareas_wave = [np.array([[0, 0], [0.1, 0], [0.1, 0.1], [0, 0.1]])]

"Statistical analysis generation"
"--------------------------------"
B = np.array([0.0, 270.0]) / 180 * np.pi
H = np.array([1.0])
T = np.array([4.0])
...
x = np.linspace(0.0, 1000.0, 75)
y = np.linspace(0.0, 300.0, 11)
...
dirs = np.array([0, 120, 240], "f") / 180 * np.pi
...
if not iWP2input.stopWP2run:
    WPobj = WP2(
        iWP2input, Cfit=np.array([0.0]), Kfit=np.array([0.0]), pickup=True
    )
