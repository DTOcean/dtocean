...
if is_high_dpi() or TYPE_CHECKING:
    from dtocean_app.designer.high.multisensitivity import (
        Ui_MultiSensitivityWidget,
    )
else:
    from dtocean_app.designer.low.multisensitivity import (
        Ui_MultiSensitivityWidget,
    )
...
class MultiSensitivityWidget(
    ...
    def _get_var_id(self, var_name):
        if self._var_ids is None:
            raise IndexError(f"ID {var_name} not recognised")
    ...
    def _remove_row(self):
        ...
        self.tableView.model().removeRow(selected_row.row())
    ...
    def get_configuration(self):
        ...
        model = self.tableView.model()
        assert isinstance(model, SimTableModel)
        array_df = model.array_df
        assert isinstance(array_df, pd.DataFrame)

        def get_var_id(x):
            assert self._var_ids is not None
            return self._var_ids[x]

        df = array_df.copy()
        df["Variable"] = df["Variable"].apply(get_var_id)
...
class SimTableModel(QAbstractTableModel):
    ...
    def _init_df(self, init_records=None) -> pd.DataFrame:
        new_df = pd.DataFrame(columns=self.header_labels)

        if init_records is None:
            return new_df

        return self._add_records(new_df, init_records)
    ...
    def insertRows(self, position, records, rows=1, index=QModelIndex()):
    ...
    def data(self, index, role):
        ...
        return self.array_df.iloc[index.row(), index.column()]
