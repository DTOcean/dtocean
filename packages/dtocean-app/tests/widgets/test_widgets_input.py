...
def test_InputDictTable(qtbot):
    window = InputDictTable(
        units=["test1", "test2"], fixed_index_names=["a", "b", "c"]
    )
    window.show()
    qtbot.addWidget(window)
...
def test_InputDictTable_get_result(qtbot):
    ...
    df_dict = {"Key": raw_dict.keys(), "Value": raw_dict.values()}
...
def test_InputPointTable_get_result_znone(qtbot):
    raw_dict = {"x": [1, 2, 3], "y": [1, 2, 3], "z": [None, None, None]}
