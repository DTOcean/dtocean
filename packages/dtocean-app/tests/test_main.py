...
def core(mocker):
    mocker.patch(
        "dtocean_core.extensions.ToolManager._discover_classes",
        return_value={"MockTool": MockTool},
        autospec=True,
    )

    mocker.patch(
        "dtocean_core.extensions.ToolManager._discover_names",
...
def test_close_open_dock(qtbot, window):
    ...
    def dock_is_closed():
        assert window._system_dock.isHidden()

    qtbot.waitUntil(dock_is_closed)

    # Reopen the system dock
    menu_click(qtbot, window, window.menuView, "actionSystem_Log")
...
def test_project_open_error(mocker, qtbot, tmp_path, window_debug):
    ...
    mocker.patch.object(
        ...
        "getOpenFileName",
        return_value=(dto_file_path, None),
    )
...
def window_new_project(mocker, qtbot, window_debug):
    ...
    qtbot.mouseClick(new_project_button, Qt.MouseButton.LeftButton)
...
def test_click_section_twice(qtbot, window_new_project):
    # Pick up the available pipeline item
    test_var = window_new_project._pipeline_dock._find_controller(
        controller_title="Configuration", controller_class=SectionControl
    )
    ...
    qtbot.mouseClick(
        tree_view.viewport(), Qt.MouseButton.LeftButton, pos=rect.topLeft()
    )
...
def db_selector(qtbot, window_new_project):
    ...
    database_button = window_new_project.scenarioToolBar.widgetForAction(
        window_new_project.actionSelect_Database
    )
    qtbot.mouseClick(database_button, Qt.MouseButton.LeftButton)

    db_selector = window_new_project._db_selector

    def db_selector_visible():
        assert db_selector.isVisible()
    ...
    qtbot.mouseClick(db_selector.addButton, Qt.MouseButton.LeftButton)

    def added_cred():
        assert db_selector.listWidget.count() == n_creds + 1
...
def test_credentials_add_delete(qtbot, mocker, db_selector):
    ...
    while db_selector.listWidget.count() > 0:
        ...
        qtbot.mouseClick(
            db_selector.listWidget.viewport(),
            Qt.MouseButton.LeftButton,
            pos=rect.center(),
        )

        qtbot.mouseClick(db_selector.deleteButton, Qt.MouseButton.LeftButton)
...
def test_credentials_rename(qtbot, mocker, db_selector):
    ...
    def check_one_bob():
        assert (
            db_selector._data_menu.get_available_databases().count("bob") == 1
        )
...
def test_credentials_save(qtbot, mocker, db_selector):
    ...
    def added_cred():
        assert db_selector.listWidget.count() == old_count + 1

    qtbot.waitUntil(added_cred)

    # Select the last in the list and chnage its name
    test_cred = db_selector.listWidget.item(db_selector.listWidget.count() - 1)
    ...
    item = QtWidgets.QTableWidgetItem(str("bob"))
    db_selector.tableWidget.setItem(0, 1, item)

    def can_save():
        assert db_selector.saveButton.isEnabled()

    qtbot.waitUntil(can_save)

    # Press the save button
    qtbot.mouseClick(db_selector.saveButton, Qt.MouseButton.LeftButton)

    def can_not_save():
        assert not db_selector.saveButton.isEnabled()
...
def test_dump_load_database(
    ...
    mocker.patch(
        "dtocean_app.menu.QtWidgets.QFileDialog.getExistingDirectory",
...
def test_export_data(qtbot, mocker, tmpdir, window_floating_wave):
    ...
    mocker.patch.object(
        QtWidgets.QFileDialog,
        "getSaveFileName",
        return_value=(datastate_file_path, None),
    )
...
def window_with_pipeline(qtbot, window_floating_wave):
    ...
    init_pipeline_button = window_floating_wave.scenarioToolBar.widgetForAction(
        window_floating_wave.actionInitiate_Pipeline
    )
    qtbot.mouseClick(init_pipeline_button, Qt.MouseButton.LeftButton)

    data_check = window_floating_wave._data_check

    def data_check_visible():
        assert data_check.isVisible()

    qtbot.waitUntil(data_check_visible)

    qtbot.mouseClick(
        data_check.buttonBox.button(
            QtWidgets.QDialogButtonBox.StandardButton.Ok
        ),
        Qt.MouseButton.LeftButton,
    )
...
def window_dataflow_empty(qtbot, window_with_pipeline):
    ...
    init_pipeline_button = window_with_pipeline.scenarioToolBar.widgetForAction(
...
def window_dataflow_module(qtbot, window_with_pipeline):
    # Add a module
    add_modules_button = window_with_pipeline.simulationToolBar.widgetForAction(
        window_with_pipeline.actionAdd_Modules
    )
    qtbot.mouseClick(add_modules_button, Qt.MouseButton.LeftButton)

    module_shuttle = window_with_pipeline._module_shuttle

    def add_modules_visible():
        assert module_shuttle.isVisible()
    ...
    # Click "Add" then "OK"
    qtbot.mouseClick(module_shuttle.addButton, Qt.MouseButton.LeftButton)

    def module_on_right():
        assert module_shuttle._get_right_data()

    qtbot.waitUntil(module_on_right)

    mod_ok_button = module_shuttle.buttonBox.button(
        QtWidgets.QDialogButtonBox.StandardButton.Ok
    )
    qtbot.mouseClick(mod_ok_button, Qt.MouseButton.LeftButton)

    def add_modules_not_visible():
        assert not module_shuttle.isVisible()
...
def test_pipeline_context_menu(mocker, qtbot, window_dataflow_module):
    ...
    mocker.patch("dtocean_app.pipeline.QtWidgets.QMenu", return_value=menu)

    pipeline_dock = window_dataflow_module._pipeline_dock
    mod_control = pipeline_dock._find_controller(
        controller_title="Mock Module", controller_class=InputBranchControl
    )
...
def test_simulation_context_menu(mocker, qtbot, window_dataflow_module):
    menu = mocker.MagicMock()
    mocker.patch("dtocean_app.simulation.QtWidgets.QMenu", return_value=menu)
    ...
    QtWidgets.QApplication.postEvent(
        simulation_dock.listWidget.viewport(), event
    )
...
def test_plot_context_clear(qtbot, window_plot_context):
    controller = window_plot_context._pipeline_dock._find_controller(
        controller_title="Bathymetry", controller_class=InputVarControl
    )
...
def window_dataflow_theme(qtbot, window_with_pipeline):
    # Add a theme
    add_themes_button = window_with_pipeline.simulationToolBar.widgetForAction(
        window_with_pipeline.actionAdd_Assessment
    )
    qtbot.mouseClick(add_themes_button, Qt.MouseButton.LeftButton)

    assessment_shuttle = window_with_pipeline._assessment_shuttle

    def add_themes_visible():
        assert assessment_shuttle.isVisible()
    ...
    # Click "Add" then "OK"
    qtbot.mouseClick(assessment_shuttle.addButton, Qt.MouseButton.LeftButton)

    def module_on_right():
        assert assessment_shuttle._get_right_data()
    ...
    def add_themes_not_visible():
        assert not assessment_shuttle.isVisible()
...
def test_simulation_clone_set_title_fail(mocker, qtbot, window_two_simulations):
...
def test_simulation_clone_select(qtbot, window_two_simulations):
    ...
    qtbot.mouseClick(
        simulation_dock.listWidget.viewport(),
        Qt.MouseButton.LeftButton,
        pos=rect.center(),
    )
...
def test_theme_simulation_clone_select(qtbot, window_two_theme_simulations):
...
def test_simulation_active_themes_warn(
...
def test_modify_variable(qtbot, window_dataflow_module):
    ...
    test_var = window_dataflow_module._pipeline_dock._find_controller(
        controller_title="Device Rated Power", controller_class=InputVarControl
    )
    ...
    def is_float_select():
        assert isinstance(
            window_dataflow_module._data_context._bottom_contents, FloatSelect
        )
...
def test_strategy_select(qtbot, window_dataflow_module):
    ...
    qtbot.mouseClick(add_strategy_button, Qt.MouseButton.LeftButton)
...
def test_strategy_no_modules(qtbot, window_dataflow_theme):
    ...
    qtbot.mouseClick(
        strategy_manager.listWidget.viewport(),
        Qt.MouseButton.LeftButton,
        pos=rect.center(),
    )
    ...
    qtbot.mouseClick(strategy_manager.applyButton, Qt.MouseButton.LeftButton)
    ...
    # Close the dialog
    qtbot.mouseClick(strategy_manager.closeButton, Qt.MouseButton.LeftButton)
...
def test_strategy_reload(
    ...
    # Close the dialog
    qtbot.mouseClick(
        strategy_manager_basic.closeButton, Qt.MouseButton.LeftButton
    )
...
def menu_click(qtbot, main_window, menu, action_name):
    ...
    errStr = "Action '{}' not found in menu '{}'".format(
        action_name, menu.objectName()
    )
