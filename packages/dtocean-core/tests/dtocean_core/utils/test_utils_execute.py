...
def test_main_full(mocker, capsys, tmpdir):
    mocker.patch("dtocean_core.utils.execute.start_logging", autospec=True)

    mocker.patch("dtocean_core.utils.execute.Core", new=MockCore)
...
def test_main_next(mocker, capsys, tmpdir):
    ...
    mocker.patch("dtocean_core.utils.execute.ModuleMenu", new=MockMenu)
