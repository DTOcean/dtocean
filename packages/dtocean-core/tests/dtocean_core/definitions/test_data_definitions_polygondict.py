...
def test_PolygonDict():
    ...
    raw = {
        ...
        "block 2": [(10.0, 10.0), (11.0, 11.0), (12.0, 12.0)],
    }
    ...
    assert b["block 1"] is not None
    assert b["block 1"].exterior.coords[0][0] == 0.0

    assert b["block 2"] is not None
    assert b["block 2"].exterior.coords[2][1] == 12.0

    raw = {
        "block 1": [(0.0, 0.0, 0.0), (1.0, 1.0, 1.0), (2.0, 2.0, 2.0)],
        "block 2": [(10.0, 10.0, 10.0), (11.0, 11.0, 11.0), (12.0, 12.0, 12.0)],
    }
    ...
    assert b is not None
    assert b["block 1"] is not None
    assert b["block 1"].has_z
    assert b["block 1"].exterior.coords[0][0] == 0.0
    assert b["block 1"].exterior.coords[1][2] == 1.0  # type: ignore
    ...
    raw = {
        "block 1": [(0.0, 0.0, 0.0), (1.0, 1.0, 1.0)],
        "block 2": [(10.0, 10.0, 10.0), (11.0, 11.0, 11.0)],
    }
...
def test_PolygonDict_auto_file(tmpdir, fext):
    ...
    for raw, ztest in zip(raws, ztests):
        ...
        assert result["block 1"].exterior.coords[0][0] == 0.0
        assert result["block 2"].exterior.coords[2][1] == 12.0
...
def test_PolygonDictColumn_auto_db(mocker):
    ...
    for raw in raws:
        ...
        assert result["block 1"].exterior.coords[0][0] == 0.0
        assert result["block 2"].exterior.coords[2][1] == 11.0
