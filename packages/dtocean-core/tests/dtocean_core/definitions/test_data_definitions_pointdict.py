...
def test_PointDict():
    ...
    assert b["one"] is not None
    assert b["one"].x == 0.0

    assert b["two"] is not None
    assert b["two"].y == 1.0
    ...
    assert b["three"] is not None
    assert b["three"].z == 2.0
...
def test_PointDict_auto_file(tmpdir, fext):
    ...
    for raw, ztest in zip(raws, ztests):
        ...
        assert result["one"].x == 0.0
        assert result["two"].y == 1.0
...
def test_PointDictColumn_auto_db(mocker):
    names = ["one", "two"]
    raw_data = [
        [WKTElement("POINT (0 0)"), WKTElement("POINT (1 1)")],
