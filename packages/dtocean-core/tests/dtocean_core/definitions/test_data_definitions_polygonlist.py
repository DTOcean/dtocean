...
def test_PolygonList():
    ...
    assert b[0].exterior.coords[0][0] == 0.0

    assert b[1] is not None
    assert b[1].exterior.coords[2][1] == 12.0
    ...
    assert b[0].has_z
    assert b[0].exterior.coords[0][0] == 0.0
    assert b[0].exterior.coords[1][2] == 1.0  # type: ignore
...
def test_PolygonList_auto_file(tmpdir, fext):
    ...
    for raw, ztest in zip(raws, ztests):
        ...
        assert result[0].exterior.coords[0][0] == 0.0
        assert result[1].exterior.coords[2][1] == 12.0
...
def test_PolygonListColumn_auto_db(mocker):
    ...
    for raw in raws:
        ...
        assert result[0].exterior.coords[0][0] == 0.0
        assert result[1].exterior.coords[2][1] == 11.0
