...
def test_HistogramDict_equals():
    ...
    a = {"test_one": (values_one, bins_one), "test_two": (values_two, bins_two)}

    b = {"test_one": (values_one, bins_one), "test_two": (values_two, bins_two)}
...
def test_HistogramDict_auto_file(tmpdir, fext):
    ...
    assert (
        len(result["test_two"]["bins"]) == len(result["test_two"]["values"]) + 1
    )
