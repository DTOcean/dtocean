...
def test_NumpyLineDict():
    ...
    assert "Sin(x)" in b
    assert b["Sin(x)"] is not None
    assert max(b["Sin(x)"][:, 1]) == 1

    assert "Cos(x)" in b
    assert b["Cos(x)"] is not None
...
def test_NumpyLineDict_equals():
    ...
    raw = {"Sin(x)": np.array(coarse_sin), "Cos(x)": np.array(fine_cos)}
...
def test_NumpyLineDict_not_equal_values():
    ...
    left = {"Sin(x)": np.array(coarse_sin), "Cos(x)": np.array(fine_cos)}
    right = {"Sin(x)": np.array(fine_cos), "Cos(x)": np.array(coarse_sin)}
...
def test_NumpyLineDict_not_equal_keys():
    ...
    left = {"Sin(x)": np.array(coarse_sin), "Cos(x)": np.array(fine_cos)}
    right = {"Sin(x)": np.array(coarse_sin), "Cosh(x)": np.array(fine_cos)}
...
def test_NumpyLine_auto_file(tmpdir, fext):
    ...
    raw = {"Sin(x)": coarse_sin, "Cos(x)": fine_cos}
