...
def test_get_moorings_interface(module_menu, core, tidal_project, var_tree):
    ...
    moorings_branch = var_tree.get_branch(core, project, mod_name)
    moorings_branch.read_test_data(
        core, project, os.path.join(dir_path, "inputs_wp4.pkl")
    )
...
def test_moorings_interface_entry(
    ...
    mocker.patch(
        "dtocean_core.interfaces.moorings.UserDataPath",
