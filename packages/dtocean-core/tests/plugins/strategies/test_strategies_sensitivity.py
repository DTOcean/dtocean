...
def test_unit_configure(unit):
    ...
    assert unit._config == {"module_name":  "a",
                            "var_name": "b",
                            "var_values": [1, 2, 3],
                            "skip_errors": False}
...
def test_unit_execute(mocker, unit):
    ...
    mocker.patch.object(unit._module_menu,
                        'get_available',
    ...
    mocker.patch.object(unit._tree,
                        'get_branch',
                        return_value=mock_branch,
                        autospec=True)
    
    mocker.patch.object(unit,
                        '_safe_exe',
                        return_value=True,
                        autospec=True)
    ...
    assert unit._sim_record == ['Mock = 1 (m)',
                                'Mock = 1 (m) [repeat 1]',
                                'Mock = 2 (m)']
...
def test_unit_safe_exe(mocker, unit, skip_errors):
    ...
    mocker.patch.object(unit._basic,
                        'execute',
                        autospec=True)
