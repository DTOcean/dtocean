...
from dtocean_plugins.strategies.multi import MultiSensitivity  # pylint: disable=no-name-in-module
...
def inputs_df():
    data = {
        "Module": ["Hydrodynamics", "Electrical Sub-Systems"],
        "Variable": ["device.power_rating", "device.power_factor"],
        "Values": [(1, 2, 3), (4, 5)],
    }
...
@pytest.mark.parametrize("subsp_ratio, expected", [(1, 6), (0.5, 3)])
...
def test_multi_execute(mocker, multi, inputs_df):
    ...
    mocker.patch.object(
        multi._module_menu, "get_available", return_value=modules, autospec=True
    )

    mocker.patch.object(multi._tree, "get_branch", autospec=True)

    mocker.patch.object(multi, "_safe_exe", return_value=True, autospec=True)
...
def test_multi_safe_exe(mocker, multi, skip_errors):
    mocker.patch.object(multi, "_run_simulation", autospec=True)
...
def test_multi_safe_exe_raise(mocker, multi):
    mocker.patch.object(
        multi, "_run_simulation", side_effect=SystemExit("foo"), autospec=True
    )
...
def test_multi_safe_exe_pass(mocker, multi):
    mocker.patch.object(
        multi, "_run_simulation", side_effect=ValueError("foo"), autospec=True
    )
...
def test_multi_run_simulation(mocker, multi):
    ...
    mocker.patch.object(
        multi._module_menu, "get_active", return_value=modules, autospec=True
    )

    mock_branch = mocker.MagicMock()
    mock_branch.get_input_status.return_value = {
        "device.power_rating": "",
        "device.power_factor": "",
    }

    mocker.patch.object(
        multi._tree, "get_branch", return_value=mock_branch, autospec=True
    )
