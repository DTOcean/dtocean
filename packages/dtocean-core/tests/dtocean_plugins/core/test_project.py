...
def test_options_interface_sites(core, wave_project):
    ...
    assert available_sites is not None
    ...
    assert not interface.data.corridor_selected
    assert not interface.data.lease_selected
...
def test_options_interface_systems(core, wave_project):
    ...
    assert available_systems is not None
...
def test_boundaries_interface(core, wave_project):
    ...
    projection = boundaries_branch.get_input_variable(
        core, project, "site.projection"
    )
