...
def test_advanced_get_project_status_not_activated():
    ...
    (status_str,
     status_code) = AdvancedPosition.get_project_status(mock_core,
                                                        mock_project,
                                                        mock_config)
...
def test_advanced_get_project_status_not_contain_module(mocker):
    ...
    mocker.patch("dtocean_core.strategies.position.ModuleMenu.get_active",
                 return_value=["mock"],
                 autospec=True)
...
def test_advanced_prepare_project_active_simulation(caplog, mocker, advanced):
    ...
    mocker.patch("dtocean_core.strategies.position.Tree",
                 autospec=True)
...
def test_advanced_prepare_project_reset_level(caplog, mocker, advanced):
    ...
    mock_core.register_level(mock_project,
                             mock_core._markers["initial"],
                             None)
    ...
    mock_core.control.set_interface_completed(mock_sim,
                                              'modules',
                                              interface_name)
...
def test_advanced_pre_execute_restart(caplog, mocker, advanced):
    ...
    mock_restart = mocker.patch("dtocean_core.strategies.position."
                                "PositionOptimiser.restart",
                                autospec=True)
...
def test_advanced_pre_execute_start(mocker, advanced):
    ...
    mock_start = mocker.patch("dtocean_core.strategies.position."
                              "PositionOptimiser.start",
                              autospec=True)
...
def test_run_favorite_success(mocker):
    ...
    mocker.patch("dtocean_core.strategies.position.logging.disable",
                 autospec=True)
    ...
    mocker.patch.object(mock_opt,
                        "get_es",
                        autospec=True)
    
    mocker.patch.object(mock_opt,
                        "get_nh",
                        return_value=mock_nh,
                        autospec=True)
    
    mock_cma_main = mocker.patch("dtocean_core.utils.optimiser.Main")
    mocker.patch.object(mock_cma_main,
    ...
    mock_evaluator = mocker.patch("dtocean_core.strategies.position_optimiser."
                                  "PositionEvaluator",
                                  autospec=True)
    ...
    mock_iterate = mocker.patch("dtocean_core.strategies.position.iterate",
                                autospec=True)
    
    mock_write_result_file = mocker.patch("dtocean_core.strategies.position."
                                          "write_result_file",
                                          autospec=True)
    ...
    assert mock_write_result_file.call_args.args[3] == {'theta': 1.0,
                                                        'dr': 2,
                                                        'dc': 3,
                                                        'n_nodes': 4,
                                                        't1': 5,
...
def test_run_favorite_exception(mocker):
    ...
    mocker.patch("dtocean_core.strategies.position.iterate",
...
def test_read_yaml_exception(mocker):
    ...
    mocker.patch('dtocean_core.strategies.position.open',
...
def test_post_process(caplog, tmpdir):
    ...
    assert data == {'sim_number': sim_num,
                    'n_nodes': n_nodes_val,
                    'mock': mock_val,
                    'objective': mock_val}
...
def test_advanced_post_process(mocker, advanced):
    ...
    mocker.patch("dtocean_core.strategies.position._run_favorite",
                 autospec=True)
    mocker.patch("dtocean_core.strategies.position._post_process",
                 autospec=True)
...
def test_get_results_table():
    ...
    mock_dict_val = [{"sub_mock1": x,
                      "sub_mock2": x} for x in mock_val]
    
    results_dict = {'sim_number': sim_num,
                    'n_nodes': n_nodes_val,
                    'mock': mock_val,
                    'objective': mock_val,
    ...
    assert len(result) == 5
    assert list(result.columns) == ['sim_number',
                                    'objective',
                                    'grid_orientation',
                                    'n_nodes',
                                    'mock_dict [sub_mock1]',
    ...
    assert np.isclose(result['grid_orientation'].values,
                      [90, 0, 270, 180, 90]).all()
...
def test_get_results_table_single():
    ...
    mock_dict_val = {"sub_mock1": 10,
                     "sub_mock2": 10}
    
    results_dict = {'sim_number': 0,
                    'n_nodes': 15,
                    'mock': 10,
                    'objective': 10,
                    'grid_orientation': 0,
...
def test_advanced_import_simulation_file(mocker, advanced, results_dict):
    ...
    mocker.patch('dtocean_core.strategies.position.open',
                 mocker.mock_open(read_data=dump(results_dict,
                                                 Dumper=Dumper)))
    ...
    mocker.patch('dtocean_core.strategies.position.get_positioner',
                 autospec=True)
    
    mocker.patch('dtocean_core.strategies.position.prepare',
                 autospec=True)
...
def test_advanced_load_simulation_ids(mocker,
                                      tmpdir,
                                      advanced,
                                      results_dict):
    ...
    assert advanced.get_simulation_record() == ['Simulation 0', 
                                                'Simulation 1',
                                                'Simulation 2',
                                                'Simulation 3',
                                                'Simulation 4']
...
def test_advanced_load_simulation_ids_titles(mocker,
                                             tmpdir,
                                             advanced,
                                             results_dict):
