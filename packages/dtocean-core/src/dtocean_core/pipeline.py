...
class Tree(object):
    ...
    def get_available_branches(self, core, project, hub_names=None):
        ...
        all_branch_names = [y for x in hub_branch_names.values() for y in x]
    ...
    def get_branch(self, core, project, branch_name):
        ...
        for hub_name, branch_names in hub_branch_names.items():
    ...
    def read(
        ...
        branch_list = [
            self.get_branch(core, project, name) for name in branch_names
        ]
        ...
        for branch in branch_list:
            ...
            if skip_unavailable:
                branch_inputs = [
                    k
                    for k, v in branch_input_status.items()
        ...
        _read_variables(
            core, project, variables, overwrite, set_auto, log_exceptions
        )
    ...
    def _get_branch_names(self, core, project, hub_names=None):
        ...
        for hub_name in hub_names:
            ...
            branch_names = connector.get_active_interface_names(core, project)
...
class Branch(object):
    ...
    def get_inputs(self, core, project):
        ...
        input_declaration = connector.get_interface_inputs(
            core, project, self._name
        )
    ...
    def get_outputs(self, core, project):
        connector = _get_connector(project, self._hub_name)
        output_declaration = connector.get_interface_outputs(
            core, project, self._name
        )
    ...
    def get_input_status(self, core, project, variable=None):
        ...
        if variable is not None:
            if not isinstance(variable, InputVariable):
                errStr = (
                    "Argument 'variable' only accepts InputVariable " "objects."
                )
                raise ValueError(errStr)

            if variable._id not in branch_status:
                errStr = (
                    "Variable {} is not an input to branch " "{}."
    ...
    def get_output_status(self, core, project, variable=None):
        ...
        branch_status = connector.get_interface_outputs_status(
            project, self._name
        )
        ...
        if variable is not None:
            if not isinstance(variable, OutputVariable):
                errStr = (
                    "Argument 'variable' only accepts OutputVariable "
                    "objects."
                )
                raise ValueError(errStr)
            ...
            if variable._id not in branch_status:
                errStr = (
                    "Variable {} is not an output of branch " "{}."
                ).format(variable._id, self._name)
                raise ValueError(errStr)
    ...
    def get_output_variable(self, core, project, variable_id):
        ...
        # Check that the inputs exist in the tree
        module_inputs = self.get_output_status(core, project)

        if variable_id not in module_inputs.keys():
            errStr = ("Variable {} is not an output of branch " "{}.").format(
                variable_id, self._name
            )
    ...
    def inspect(self, core, project, scope=None):
        ...
        if not connector.is_interface_completed(core, project, self._name):
            return

        output_level = "{} {}".format(
            self._name.lower(), core._markers["output"]
        )
        register_level = "{} {}".format(
            self._name.lower(), core._markers["register"]
        )
        ...
        set_output_scope(core, project, scope)
    ...
    def read(
        ...
        if self._hub_name in ["modules", "themes"] and not core.has_data(
            project, "hidden.dataflow_active"
        ):
            errStr = (
        ...
        for identifier, status in branch_input_status.items():
            if skip_unavailable and "unavailable" in status:
                continue

            var = self.get_input_variable(core, project, identifier)
    ...
    def read_auto(
        ...
        self.read(
            core, project, overwrite, skip_unavailable, True, log_exceptions
        )
    ...
    def read_test_data(
        ...
        for variable_id, value in test_data.items():
            variable = self.get_input_variable(
                core, project, variable_id, skip_missing=skip_missing
            )
...
class Variable(object):
    ...
    def get_file_output_interfaces(self, core, project, include_auto=False):
        ...
        if include_auto:
            kargs["auto_interface_name"] = "AutoFileOutput"
    ...
    def get_available_plots(self, core, project, include_auto=False):
        ...
        if include_auto:
            args.append("AutoPlot")

        active_receivers = self._get_receivers(*args)
        assert isinstance(active_receivers, list)
        active_receivers.sort()

        return active_receivers

    def write_file(self, core, project, file_path, interface_name=None):
        interface = self._get_receiving_interface(
            core,
            project,
            "FileOutputInterface",
            "AutoFileOutput",
        ...
        if interface is None or not core.has_data(project, self._id):
            return
    ...
    def plot(self, core, project, plot_name=None):
        interface = self._get_receiving_interface(
            core, project, "PlotInterface", "AutoPlot", plot_name
        )
    ...
    def _get_named_interface(
        self, core, socket_cls_name, interface_name, allow_missing=False
    ):
        ...
        if interface_name in interface_name_map:
            ...
            errStr = ("Interface {} not found for socket " "class {}").format(
                interface_name, socket_cls_name
            )
            raise ValueError(errStr)

        return interface

    def _find_providing_interface(
        self, core, socket_cls_name, allow_missing=False, allow_multiple=False
    ):
        interfaces = self._find_providing_interfaces(core, socket_cls_name)
        interface = self._select_interface(
            interfaces, socket_cls_name, allow_missing, allow_multiple
        )

        return interface

    def _find_receiving_interface(
        self, core, socket_cls_name, allow_missing=False, allow_multiple=False
    ):
    ...
    def _get_providers(
        ...
        if auto_interface_name is None:
            ...
            providers.extend(auto_providers)
        ...
        result = {
            name: value for name, value in zip(provider_names, provider_values)
        }
    ...
    def _get_receivers(
        ...
        if auto_interface_name is not None:
            receivers = self._find_receiving_interfaces(
                core, auto_interface_name
            )
        ...
        result = {
            name: value
            for name, value in zip(active_receiver_names, receiver_values)
        }

        return result

    def _get_providing_interface(
        self,
        core,
        ...
        if interface_name is not None:
            if interface_name in self._get_providers(
                core, super_interface_name
            ):
                interface = self._get_named_interface(
                    core, super_interface_name, interface_name
                )

            elif (
                auto_interface_name is not None
                and interface_name
                in self._get_providers(core, auto_interface_name)
            ):
                interface = self._get_named_interface(
                    core, auto_interface_name, interface_name
                )

            else:
                errStr = (
                    "Interface {} is not available for " "variable {}."
                ).format(interface_name, self._id)
                raise ValueError(errStr)

        if interface is None and auto_interface_name is not None:
            interface = self._find_providing_interface(
                core, auto_interface_name, allow_missing=True
            )
    ...
    def _get_receiving_interface(
        ...
        if interface_name is not None:
            if interface_name in self._get_receivers(
                core, project, super_interface_name
            ):
                interface = self._get_named_interface(
                    core, super_interface_name, interface_name
                )
                ...
                auto_interface_name is not None
                and interface_name
                in self._get_receivers(core, project, auto_interface_name)
            ):
                interface = self._get_named_interface(
                    core, auto_interface_name, interface_name
                )
        ...
        if interface is None and auto_interface_name is not None:
            interface = self._find_receiving_interface(
                core, auto_interface_name, allow_missing=True
            )
    ...
    def _select_interface(
        ...
        if len(interfaces) == 0 and allow_missing:
            ...
            errStr = (
                "No interfaces found of type {} for variable " "{}."
            ...
            errStr = (
                "Multiple interfaces of type {} found for variable {}: " "{}."
    ...
    @classmethod
    def _get_socket_interfaces(cls, socket, interface_cls_names):
        interfaces = []
...
class InputVariable(Variable):
    ...
    def set_raw_interface(self, core, value, interface_name=None):
        interface = self._get_providing_interface(
            core, "RawInterface", "AutoRaw", interface_name
        )

        if interface is None:
            errStr = ("No raw input interface found for " "variable ").format()
            raise RuntimeError(errStr)
    ...
    def set_query_interface(self, core, project, interface_name=None):
        # Check the status of the database.
        if project.get_database_credentials() is None:
            errStr = "No database credentials found"
            raise RuntimeError(errStr)

        interface = self._get_query_interface(core, project, interface_name)

        if interface is None:
            errStr = (
                "No database access interface found for " "variable "
            ).format()
            raise RuntimeError(errStr)
    ...
    def read(
        ...
        _read_variables(
            core, project, [self], overwrite, set_auto, log_exceptions
        )

    def read_auto(self, core, project, overwrite=True, log_exceptions=True):
        """Calls read with set_auto set to True."""
    ...
    def _get_query_interface(self, core, project, interface_name=None):
        interface = self._get_providing_interface(
            core, "QueryInterface", "AutoQuery", interface_name
        )
        ...
        # auto recovery.
        if (
            not metadata.auto_only
            or metadata.auto_only
            and core.has_data(project, metadata.auto_only)
        ):
            result = interface
...
def set_output_scope(
    ...
    level_root = simulation.level_map[simulation.get_inspection_level()].lower()

    unmask_level = "{} {}".format(level_root, unmask_marker)
    ...
    core.unmask_states(project, simulation, unmask_level, update_status=False)
...
def _get_connector(project, hub_name):
    ...
    if hub_name not in hub_names:
        errStr = ('Hub name "{}" is not contained in the ' "project.").format(
            hub_name
        )
        raise ValueError(errStr)
...
def _can_read_value(core, project, variable_id, overwrite=True):
    result = True
...
def _get_read_values(core, project, interface, variable_ids):
    if interface is None:
...
def _read_variables(
    ...
    while variables:
        ...
        if not _can_read_value(core, project, variable._id, overwrite):
            continue
        ...
        if set_auto:
            ...
            interface = variable._get_query_interface(core, project)
        ...
        for add_var_id in add_var_ids:
            ...
            if (
                not set_auto
                and add_var_interface_name is not None
                and variable_interface_name != add_var_interface_name
            ):
                msg = (
                    "Variable {} is being retrieved from interface {} "
                    "rather than {}"
                ).format(
                    add_var_id, add_var_interface_name, variable_interface_name
                )
                module_logger.warning(msg)

            if not _can_read_value(core, project, add_var_id, overwrite):
                continue
        ...
        if log_exceptions:
            try:
                values = _get_read_values(
                    core, project, interface, fetch_var_ids
                )

            except:  # noqa: E722
                e = sys.exc_info()[0]
                msg = ("Reading variables {} generated error: " "{}").format(
                    varsStr, e
                )
            ...
            values = _get_read_values(core, project, interface, fetch_var_ids)
