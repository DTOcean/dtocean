...
def get_input_tables(
    ...
    # Set defaults
    if spare_cost_multiplier is None:
        spare_cost_multiplier = 1.0
    if transit_cost_multiplier is None:
        transit_cost_multiplier = 0.0
    if loading_cost_multiplier is None:
        loading_cost_multiplier = 0.0

    # Define required subsystems

    # Device
    device_subsystems = ["Prime Mover", "PTO", "Support Structure"]
    ...
    if elec_network is not None:
        elec_subsystems = ["Inter-Array Cables", "Substations", "Export Cable"]
        ...
        if n_subhubs > 0:
            subhubs = ["subhub{:0>3}".format(x + 1) for x in range(n_subhubs)]

        electrical_subsystem_costs = get_electrical_system_cost(
            electrical_components, elec_subsystems, elec_database
        )
    ...
    if mandf_network is not None:
        ...
        mandf_subsystem_costs = get_mandf_system_cost(
            mandf_bom, mandf_subsystems, mandf_database
        )

        all_subsystems.extend(mandf_subsystems)

    standalone_subsystems = [
        "Prime Mover",
        "PTO",
        "Control",
        "Support Structure",
    ...
    weightings_map = {
        "Prime Mover": subsystem_operation_weightings.loc["Prime Mover"],
        "PTO": subsystem_operation_weightings.loc["PTO"],
        "Support Structure": subsystem_operation_weightings.loc[
            "Support Structure"
        ],
    ...
    if control_subsystem_replacement_parts is not None:
        weightings_map["Control"] = subsystem_operation_weightings.loc[
            "Control"
        ]
    ...
    if annual_maintenance_start is not None:
        ...
        start_date = dt.datetime(this_year, start_month_int, 1)
    ...
    if annual_maintenance_end is not None:
        ...
        if start_month_int is not None and end_month_int < start_month_int:
            errStr = (
                "Annual maintenance end month may not be earlier than "
                "start month"
            )
            raise ValueError(errStr)

        last_day = calendar.monthrange(this_year, end_month_int)[1]

        end_date = dt.datetime(this_year, end_month_int, last_day, 23, 59, 59)

        if end_date < maintenance_start or end_date < start_date:
            last_day = calendar.monthrange(next_year, end_month_int)[1]

            end_date = dt.datetime(
                next_year, end_month_int, last_day, 23, 59, 59
            )

    else:
        end_date = dt.datetime(next_year, 12, 31, 23, 59, 59)
    ...
    longErr = (
        "{0} {1} operations {2} data has not been provided, yet "
    ...
    for subsystem in all_subsystems:
        ...
        if (
            operations_onsite_maintenance is not None
            and operations_onsite_maintenance[subsystem]
        ):
            weighting = weightings_map[subsystem]["On-Site Maintenance"]
            operations_weightings["onsite"] = weighting

        if (
            operations_replacements is not None
            and subsystem in operations_replacements
            and operations_replacements[subsystem]
        ):
            weighting = weightings_map[subsystem]["Replacement"]
            operations_weightings["replacement"] = weighting

        if (
            operations_inspections is not None
            and operations_inspections[subsystem]
        ):
        ...
        if len(weighting_set) == 1 and None in weighting_set:
            continue
        ...
        active_ops = [
            k for k, v in operations_weightings.items() if v is not None
        ]
        total_weight = sum(
            [v for k, v in operations_weightings.items() if v is not None]
        )
        ...
        if subsystem in device_subsystems:
            temp_comp["failure_rate"] = (
                subsystem_failure_rates[subsystem] * 24 * 365 / 1e6
            )
        ...
        if "onsite" in active_ops:
            ...
            if subsystem in device_subsystems:
                ...
                if electrical_onsite_parts is None:
                    errStr = longErr.format(
                        "Electrical Sub-Systems", "on-site", "parts", subsystem
                    )
                    raise RuntimeError(errStr)
            ...
            onsite_modes["mode_probability"] = (
                operations_weightings["onsite"] / total_weight * 100.0
            )

            # Add operation to Failure Mode and Repair Actions Tables
            (all_modes, all_repair) = update_onsite_tables(
                subsystem,
                subsystem_root,
                system_type,
                array_layout,
        ...
        if "replacement" in active_ops:
            ...
            if subsystem in device_subsystems:
                ...
                temp_repair = electrical_replacement_requirements.loc[subsystem]
                replacement_modes = replacement_modes.append(
            ...
            replacement_modes["mode_probability"] = (
                operations_weightings["replacement"] / total_weight * 100.0
            )

            # Add operation to Failure Mode and Repair Actions Tables
            (all_modes, all_repair) = update_replacement_tables(
                subsystem,
                subsystem_root,
                system_type,
                transportation_method,
        ...
        if "inspections" in active_ops:
            ...
            if subsystem in device_subsystems:
                ...
                temp_inspection = subsystem_inspections.loc[subsystem]

                temp_inspection = temp_inspection.rename(device_inspections_map)
                ...
                temp_inspection = electrical_inspections_requirements.loc[
                    subsystem
                ]
                ...
                temp_inspection = moorings_inspections_requirements.loc[
                    subsystem
                ]
            ...
            # Costs
            inspections_modes["cost_spare"] = 0.0
            inspections_modes["cost_spare_transit"] = 0.0
            inspections_modes["cost_spare_loading"] = 0.0
            ...
            inspections_modes["mode_probability"] = (
                operations_weightings["inspections"] / total_weight * 100.0
            )

            # Add operation to Failure Mode and Inspection Tables
            (all_modes, all_inspection) = update_inspections_tables(
                subsystem,
                subsystem_root,
                system_type,
                array_layout,
    ...
    if all_repair.empty:
        index = [
            "Component_ID",
            "FM_ID",
            "current_speed_max_acc",
            "current_speed_max_om",
            "delay_crew",
            "delay_organisation",
            "delay_spare",
    ...
    assert all_modes.shape[1] == all_repair.shape[1] + all_inspection.shape[1]

    return all_comp, all_modes, all_repair, all_inspection


def update_comp_table(
    subsystem, subsystem_root, array_layout, temp_comp, all_comp, subhubs
):
    array_subsystems = ["Substations", "Export Cable"]
    ...
    if subsystem in array_subsystems:
        ...
        all_comp = pd.concat([all_comp, temp_comp], axis=1, sort=False)
...
def update_onsite_tables(
    ...
    array_dict = {"Substations": [1, 1], "Export Cable": [7, 3]}
    ...
    if subsystem in array_subsystems:
        ...
        all_repair = pd.concat([all_repair, temp_repair], axis=1, sort=False)
        ...
        all_modes = pd.concat([all_modes, temp_modes], axis=1, sort=False)
        ...
        if subsystem == "Substations" and subhubs is not None:
            for subhub_id in subhubs:
                ...
                num_cols = all_repair.shape[1]
                temp_repair = temp_repair.rename(num_cols)
                all_repair = pd.concat(
                    [all_repair, temp_repair], axis=1, sort=False
                )
                ...
                temp_modes = temp_modes.rename(num_cols)
                all_modes = pd.concat(
                    [all_modes, temp_modes], axis=1, sort=False
                )
...
def update_replacement_tables(
    ...
    replacement_df = pd.DataFrame(replacement_dict, index=["Fixed", "Floating"])
...
def update_inspections_tables(
    ...
    if subsystem in array_subsystems:
        ...
        all_inspection = pd.concat(
            [all_inspection, temp_inspection], axis=1, sort=False
        )
...
def get_user_network(subsytem_comps, array_layout):
    ...
    subsystem_names = ["Hydrodynamic", "Pto", "Support structure"]
    ...
    device_subsytem_hierarchy = {
        k: [v] for k, v in zip(subsystem_names, subsytem_comps)
    }

    device_subsytem_bom = {
        k: {"quantity": Counter({v: 1})}
...
def get_user_compdict(subsytem_comps, subsystem_failure_rates):
    subsystem_names = ["Prime Mover", "PTO", "Support Structure"]
    ...
    comp_db = get_component_dict("user-defined", rates_df, rates_df)
...
def get_events_table(
    raw_df, prepend_special_raw=None, prepend_special_name=None
):
    ...
    subsytem_map = {
        "Elec sub-system": "Inter-Array Cables",
        "Control": "Control",
...
def get_electrical_system_cost(component_data, system_names, db):
    ...
    for _, component in component_data.iterrows():
        ...
        if install_type not in subsytem_map.keys():
            errStr = ("Installation type '{}' is not " "recognised").format(
                install_type
            )
            raise ValueError(errStr)
        ...
        cost_dict[subsytem_type] += _get_elec_db_cost(
            component_id, component.Quantity, db, install_type
        )
    ...
    for _, component in mandf_bom.iterrows():
        ...
        if component_key == "n/a":
            ...
            if component_type not in subsytem_map.keys():
                errStr = ("Component type '{}' is not " "recognised").format(
                    component_type
                )
                raise ValueError(errStr)
...
def _get_elec_db_cost(component_key, quantity, db, type_):
    ...
    if type_ not in convert_map:
        errMsg = ("Electrical system type '{}' is not " "recognised").format(
            type_
        )
