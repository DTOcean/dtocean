...
def package_dir(src_dir_path, dst_path: StrOrPath, archive=False):
    dst_path = Path(dst_path)

    if not os.path.splitext(dst_path)[1]:
        archive = False
    ...
    def wrapper(src_path, max_attempts=60, sleep_seconds=1, fail_silent=False):
        ...
        while file_locked:
            ...
            if n_attempts > max_attempts:
                ...
                err_str = (
                    "shutil.rmtree failed for over {} seconds: " "{}"
...
def init_dir(dir_name, clean_existing=False):
    ...
    if not clean_existing:
        err_msg = (
            "Directory {} already exists. Set clean_existing "
    ...
    for entry_name in os.listdir(dir_name):
        ...
        if os.path.isfile(entry_path):
            ...
            err_str = ("Path {} has unhandled filesystem " "type").format(
                entry_path
            )
