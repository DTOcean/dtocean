...
def make_wave_statistics(
    ...
    if dT > 1 or dH > 0.5 or dD > 45:
        errStr = (
            "ERROR: The given discretisation is larger then the"
    ...
    if wave_df["Hm0"].min() < 0.0:
        errStr = (
            "Hm0 values must be greater than zero. Minimum value of "
            "given series is: {}"
        ).format(wave_df["Hm0"].min())
        raise ValueError(errStr)

    if wave_df["Te"].min() < 0.0:
        errStr = (
            "Te values must be greater than zero. Minimum value of "
            "given series is: {}"
        ).format(wave_df["Te"].min())
        raise ValueError(errStr)

    if wave_df["Dir"].min() < -360.0 or wave_df["Dir"].max() > 360.0:
        errStr = (
            "Dir values must lie between -360. and 360. Maximum and "
            "minimum of the given series are: {} and {}"
        ).format(wave_df["Dir"].max(), wave_df["Dir"].min())
        raise ValueError(errStr)

    # Correct negative directions
    wave_df.ix[wave_df.Dir < 0, "Dir"] = (
        360.0 + wave_df.ix[wave_df.Dir < 0, "Dir"]
    )

    direcs = (wave_df["Dir"]) * np.pi / 180.0
    ...
    Te_centered = xedges[:-1] + dT / 2.0
    Hm0_centered = yedges[:-1] + dH / 2.0
    Dir_centered = np.array(Dbin[0:-1], dtype=float) * 180.0 / np.pi + (
        dD / 2.0
    )
    ...
    if save_flag:
        ...
        fid.write("\n")
        fid.write(
            "Porbability of occurence [h/h_year], Te-columns and " "Hm0-rows\n"
        )
...
def add_Te(wave_df, gamma=3.3, drop_nan=True):
    ...
    if not needed_cols.issubset(given_cols):
        errStr = (
            "Columns names 'Hm0' and 'Tp' must be provided in given "
            "dataframe."
        )
        raise ValueError(errStr)
...
def make_spectra_analysis(S, w):
    ...
    Tp2 = 2.0 * np.pi * simps(S**4, w) / simps(w * S**4, w)
...
def make_power_histograms(device_power_pmfs, rated_power, bin_width=None):
...
def add_Te_interface():
    ...
    if ext == ".csv":
        ...
        new_df.to_excel(file_path, engine="openpyxl", index=False)
