...
try:
    ...
    raise ImportError("dtocean-economics is not installed")
    ...
    def connect(self, debug_entry=False):
        ...
        if self.data.energy_per_year is not None:
            ...
            energy_record = estimate_energy(
                self.data.lifetime, self.data.annual_energy, net_coeff
            )
        ...
        result = main(
            capex_bom, opex_bom, energy_record, self.data.discount_rate
        )
        ...
        for key, arg_root in args_table.iteritems():
            if metrics_table[key].isnull().any():
                continue
        ...
        if self.data.lifetime_opex_mean is not None:
            if lifetime_cost_mean is None:
                lifetime_cost_mean = 0
            lifetime_cost_mean += self.data.lifetime_opex_mean

        if self.data.lifetime_opex_mode is not None:
            if lifetime_cost_mode is None:
                lifetime_cost_mode = 0
        ...
        if len(metrics_table["Discounted Energy"]) < 3:
            mean_lcoe = (
                result["Discounted CAPEX"] / 1000.0 + np.mean(opex)
            ) / np.mean(energy)
            ...
            self.data.lcoe_mean = (
                result["Discounted CAPEX"] / 1000.0 + mean_coords[0]
            ) / mean_coords[1]

            mode_coords = distribution.mode()
            lcoe_mode = (
                result["Discounted CAPEX"] / 1000.0 + mode_coords[0]
            ) / mode_coords[1]
            ...
            if clevels:
                ...
                for discounted_opex, discounted_energy in zip(cx, cy):
                    lcoe = (
                        result["Discounted CAPEX"] / 1000.0 + discounted_opex
                    ) / discounted_energy
                    lcoes.append(lcoe)
