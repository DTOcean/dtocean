...
class MaintenanceInterface(ModuleInterface):
    ...
    def declare_id_map(self):
        ...
        id_map = {
            ...
            "operations_onsite_maintenance": "options.operations_onsite_maintenance",
            "operations_replacements": "options.operations_replacements",
            "operations_inspections": "options.operations_inspections",
            "umbilical_operations_weighting": "project.umbilical_operations_weighting",
            "array_cables_operations_weighting": "project.array_cables_operations_weighting",
            "substations_operations_weighting": "project.substations_operations_weighting",
            "export_cable_operations_weighting": "project.export_cable_operations_weighting",
            "foundations_operations_weighting": "project.foundations_operations_weighting",
            "moorings_operations_weighting": "project.moorings_operations_weighting",
            "condition_maintenance_soh": "options.condition_maintenance_soh",
            "calendar_maintenance_interval": "options.calendar_maintenance_interval",
            ...
            "subsystem_maintenance_parts": "device.subsystem_maintenance_parts",
            "subsystem_operation_weightings": "device.subsystem_operation_weightings",
            ...
            "control_subsystem_inspections": "device.control_subsystem_inspections",
            "control_subsystem_maintenance": "device.control_subsystem_maintenance",
            "control_subsystem_maintenance_parts": "device.control_subsystem_maintenance_parts",
            "control_subsystem_operation_weightings": "device.control_subsystem_operation_weightings",
            "control_subsystem_replacement": "device.control_subsystem_replacement",
            "electrical_onsite_requirements": "project.electrical_onsite_maintenance_requirements",
            "moorings_onsite_requirements": "project.moorings_onsite_maintenance_requirements",
            "electrical_replacement_requirements": "project.electrical_replacement_requirements",
            "moorings_replacement_requirements": "project.moorings_replacement_requirements",
            "electrical_inspections_requirements": "project.electrical_inspections_requirements",
            "moorings_inspections_requirements": "project.moorings_inspections_requirements",
            "electrical_onsite_parts": "project.electrical_onsite_maintenance_parts",
            "moorings_onsite_parts": "project.moorings_onsite_maintenance_parts",
            "electrical_replacement_parts": "project.electrical_replacement_parts",
            ...
            "array_availability_lower": "project.array_availability_interval_lower",
            "array_availability_upper": "project.array_availability_interval_upper",
            "array_downtime_mean": "project.array_downtime_mean",
            "array_downtime_mode": "project.array_downtime_mode",
            "array_downtime_lower": "project.array_downtime_interval_lower",
            "array_downtime_upper": "project.array_downtime_interval_upper",
            "operation_journeys_mean": "project.operation_journeys_mean",
            "operation_journeys_mode": "project.operation_journeys_mode",
            "operation_journeys_lower": "project.operation_journeys_interval_lower",
            "operation_journeys_upper": "project.operation_journeys_interval_upper",
            ...
            "condition_events_best": "project.condition_maintenance_events_best",
            "corrective_events_best": "project.corrective_maintenance_events_best",
            "calendar_events_worst": "project.calendar_maintenance_events_worst",
            "condition_events_worst": "project.condition_maintenance_events_worst",
            "corrective_events_worst": "project.corrective_maintenance_events_worst",
    ...
    def connect(self, debug_entry=False, export_data=False):
        ...
        farm_OM = {
            "calendar_based_maintenance": self.data.calendar_based_maintenance,
            "condition_based_maintenance": self.data.condition_based_maintenance,
        ...
        if self.data.electrical_network is not None:
            ...
            if any(none_check):
                errMsg = (
                    "Electrical component databases must be defined if "
        ...
        if self.data.moor_found_network is not None:
            ...
            if any(none_check):
                errMsg = (
                    "Moorings and foundations component databases must "
        ...
        subsystem_replacement = pd.concat(
            [self.data.subsystem_replacement, replacement_limits],
            axis=1,
            sort=False,
        )
        ...
        (component_df, failure_mode_df, repair_action_df, inspection_df) = (
            get_input_tables(
                self.data.system_type,
        ...
        reliability_input_dict = ReliabilityInterface.get_input_dict(self.data)
        ...
        if reliability_input_dict is None:
            ...
            RAM_param["elechier"] = reliability_input_dict[
                "electrical_network_hier"
            ]
            RAM_param["elecbom"] = reliability_input_dict[
                "electrical_network_bom"
            ]
            RAM_param["moorhier"] = reliability_input_dict[
                "moor_found_network_hier"
            ]
            RAM_param["moorbom"] = reliability_input_dict[
                "moor_found_network_bom"
            ]
        ...
        if (
            self.data.turbine_interdist is not None
            and self.data.turbine_interdist > 0.0
        ):
            subsystem_rates["Prime Mover"] *= 2
            subsystem_rates["PTO"] *= 2

        user_hierarchy, user_bom = get_user_network(
            subsytem_comps, self.data.array_layout
        )
        ...
        logistic_param = {
            ...
            "external_protection": installation_input_dict[
        ...
        AEP_per_device = [
            self.data.annual_energy_per_device[x] * 1e6 for x in dev_ids
        ]
        mean_power_per_device = {
            x: self.data.mean_power_per_device[x] * 1e6 for x in dev_ids
        }
        ...
        for device_id in dev_ids:
            ...
            array_df = pd.concat([array_df, dev_series], axis=1, sort=False)
        ...
        if self.data.electrical_network is not None:
            if self.data.substation_layout is None:
                errStr = (
                    "The substation layout must be provided alongside "
            ...
            array_df = pd.concat([array_df, array_series], axis=1, sort=False)
            ...
            if self.data.network_type == "Star":
                ...
                for (
                    subhub_id,
                    array_pos,
                ) in self.data.substation_layout.iteritems():
                    array_series["x coord"] = array_pos.coords[0][0]
                    array_series["y coord"] = array_pos.coords[0][1]

                    point_bathy = get_point_depth(
                        self.data.bathymetry, array_pos
                    )
                    ...
                    array_series = array_series.rename(num_cols)
                    array_df = pd.concat(
                        [array_df, array_series], axis=1, sort=False
                    )
        ...
        control_param = {
            ...
            "numberOfParallelActions": self.data.parallel_operations,
        ...
        if error_df["error_ID [-]"][0] == "NoSolutionsFound":
            ...
            errStr = (
                "Problem with feasibility of logistic solution: "
        ...
        name_map = {
            "lifetimeOpex [Euro]": "Lifetime OPEX",
        ...
        if not best_strategies["UnCoMa_eventsTable"].isnull().values.all():
            ...
            best_corrective_events_df = get_events_table(
                raw_df, "failureDate [-]", "Failure Date"
            )
        ...
        if not worst_strategies["UnCoMa_eventsTable"].isnull().values.all():
            ...
            worst_corrective_events_df = get_events_table(
                raw_df, "failureDate [-]", "Failure Date"
            )
