...
from ..utils.install_electrical import (
...
from ..utils.installation import (
    find_marker_key_mf,
    installation_phase_cost_output,
...
class InstallationInterface(ModuleInterface):
    ...
    def declare_outputs(cls):
        ...
        output_list = [
            "project.installation_completion_date",
    ...
    def declare_id_map(self):
        ...
        id_map = {
            ...
            "vibro_driver_safety_factors": "project.vibro_driver_safety_factors",
            "cable_burial_safety_factors": "project.cable_burial_safety_factors",
            ...
            "vessel_average_size": "project.installation_vessel_average_size",
            ...
            "device_component_costs": "project.device_phase_installation_costs",
            "device_component_cost_breakdown": "project.device_phase_installation_cost_breakdown",
            "device_cost_class_breakdown": "project.device_phase_cost_class_breakdown",
            "device_component_times": "project.device_phase_installation_times",
            "device_component_time_breakdown": "project.device_phase_installation_time_breakdown",
            "device_time_class_breakdown": "project.device_phase_time_class_breakdown",
            "electrical_component_costs": "project.electrical_phase_installation_costs",
            "electrical_component_cost_breakdown": "project.electrical_phase_installation_cost_breakdown",
            "electrical_cost_class_breakdown": "project.electrical_phase_cost_class_breakdown",
            "electrical_component_times": "project.electrical_phase_installation_times",
            "electrical_component_time_breakdown": "project.electrical_phase_installation_time_breakdown",
            "electrical_time_class_breakdown": "project.electrical_phase_time_class_breakdown",
            "mooring_component_costs": "project.mooring_phase_installation_costs",
            "mooring_component_cost_breakdown": "project.mooring_phase_installation_cost_breakdown",
            "mooring_cost_class_breakdown": "project.mooring_phase_cost_class_breakdown",
            "mooring_component_times": "project.mooring_phase_installation_times",
            "mooring_component_time_breakdown": "project.mooring_phase_installation_time_breakdown",
            "mooring_time_class_breakdown": "project.mooring_phase_time_class_breakdown",
            "total_phase_costs": "project.installation_phase_cost_breakdown",
            "total_class_costs": "project.installation_cost_class_breakdown",
            "total_costs": "project.total_installation_cost",
            "total_phase_times": "project.installation_phase_time_breakdown",
            "total_class_times": "project.installation_time_class_breakdown",
            "total_times": "project.total_installation_time",
    ...
    def connect(self, debug_entry=False, export_data=False):
        ...
        if not all(x in self.data.lease_utm_zone for x in ["utm", "zone"]):
            errStr = (
                "Site projection not correctly defined. Must contain "
            ...
            zone = utm_zone[
                utm_zone.find("zone=") + 5 : utm_zone.find(
                    " +", utm_zone.find("zone=") + 1
                )
            ]
        ...
        if self.data.mf_network is not None:
            ...
            if foundations_data is None:
                ...
                errStr = (
                    "Complete foundations data has not been provided. "
            ...
            foundation_labels = [
                "foundation" + str(n).zfill(3)
                for n in range(len(device_association))
            ]
            ...
            foundation_map = {
                "directembedment": "direct-embedment anchor",
        ...
        if (
            self.data.mf_network is not None
            and "floating" in self.data.system_type.lower()
        ):
            ...
            if missing_titles:
                ...
                errStr = (
                    "Complete moorings data has not been provided. "
                    "{} with {} {} must be "
                    "satisfied."
                ).format(var_str, title_str, all_missing)
                raise ValueError(errStr)
            ...
            for line in all_lines:
                marker = line_components[
                    line_components["Line Identifier"] == line
                ].Marker.iloc[0]
            ...
            line_interm = pd.DataFrame(
                {"Device": device_association, "Line Identifier": all_lines}
            )

            lines_df = pd.merge(
                line_data, line_interm, on="Line Identifier", how="inner"
            )
        ...
        if export_data:
            arg_dict = {"lines_df": lines_df, "foundations_df": foundations_df}
        ...
        self.data.commissioning_date = installation_output["DATE"][
            "Comissioning Date"
        ]

        ### Port data
        self.data.port = str(
            installation_output["PORT"]["Port Name & ID [-]"][0]
        )
        self.data.port_distance = installation_output["PORT"][
        ...
        self.data.vessel_average_size = installation_output["ENVIRONMENTAL"][
            "Vessel Mean Length [m]"
        ]

        ### Planning phases
        self.data.plan = installation_output["PLANNING"][
            "List of Operations [-]"
        ]
        ...
        if any("devices" in phase for phase in installed_phases):
            values = installation_output["OPERATION"]["Installation of devices"]
        ...
        if device_component_costs.empty:
            ...
            device_component_costs = device_component_costs.set_index(
                "Component"
            )
            device_component_cost_breakdown = device_component_costs.sum(
                1
            ).to_dict()
        ...
        self.data.device_cost_class_breakdown = device_cost_class_breakdown
        ...
        if device_component_times.empty:
            ...
            device_component_times = device_component_times.set_index(
                "Component"
            )
            device_component_time_breakdown = device_component_times.sum(
                1
            ).to_dict()
        ...
        self.data.device_time_class_breakdown = device_time_class_breakdown
        ...
        if electrical_component_costs.empty:
            ...
            electrical_component_costs = electrical_component_costs.set_index(
                "Component"
            )
        ...
        if electrical_component_times.empty:
            ...
            electrical_component_times = electrical_component_times.set_index(
                "Component"
            )
        ...
        if mooring_component_costs.empty:
            ...
            mooring_component_costs = mooring_component_costs.set_index(
                "Component"
            )
            mooring_component_cost_breakdown = mooring_component_costs.sum(
                1
            ).to_dict()
            mooring_cost_class_breakdown = mooring_component_costs.sum(
                numeric_only=True
            ).to_dict()
            mooring_component_costs = mooring_component_costs.reset_index()
        ...
        self.data.mooring_cost_class_breakdown = mooring_cost_class_breakdown
        ...
        if mooring_component_times.empty:
            ...
            mooring_component_times = mooring_component_times.set_index(
                "Component"
            )
            mooring_component_time_breakdown = mooring_component_times.sum(
                1
            ).to_dict()
            mooring_time_class_breakdown = mooring_component_times.sum(
                numeric_only=True
            ).to_dict()
            mooring_component_times = mooring_component_times.reset_index()
        ...
        self.data.mooring_time_class_breakdown = mooring_time_class_breakdown
        ...
        if device_component_cost_breakdown is not None:
            ...
            year = (
                device_install_finish.year
                - self.data.project_start_date.year
                + 1
            )
        ...
        if electrical_component_cost_breakdown is not None:
            ...
            year = (
                electrical_install_finish.year
                - self.data.project_start_date.year
                + 1
            )
        ...
        cost_classes = {"Equipment": 0.0, "Vessel": 0.0, "Port": 0.0}
        ...
        if device_cost_class_breakdown is not None:
            ...
            for class_name in ["Equipment", "Vessel", "Port"]:
                cost_classes[class_name] += device_cost_class_breakdown[
                    class_name
                ]
        ...
        if electrical_cost_class_breakdown is not None:
            ...
            for class_name in ["Equipment", "Vessel", "Port"]:
                cost_classes[class_name] += electrical_cost_class_breakdown[
                    class_name
                ]
        ...
        if mooring_cost_class_breakdown is not None:
            ...
            for class_name in ["Equipment", "Vessel", "Port"]:
                cost_classes[class_name] += mooring_cost_class_breakdown[
                    class_name
                ]
        ...
        if not record_classes:
            ...
            assert np.isclose(
                cost_classes["Equipment"],
                installation_output["COST"]["Total Equipment Cost [EUR]"],
            )

            assert np.isclose(
                cost_classes["Vessel"],
                installation_output["COST"]["Total Vessel Cost [EUR]"],
            )

            assert np.isclose(
                cost_classes["Port"],
                installation_output["COST"]["Total Port Cost [EUR]"],
            )
        ...
        if phase_costs is not None and cost_classes is not None:
            ...
            assert np.isclose(
                total_costs,
                installation_output["COST"]["Total Installation Cost [EUR]"],
            )
        ...
        if device_component_time_breakdown is not None:
            phase_times["Devices"] = sum(
                device_component_time_breakdown.values()
            )

        if electrical_component_time_breakdown is not None:
            phase_times["Electrical Sub-Systems"] = sum(
                electrical_component_time_breakdown.values()
            )

        if mooring_component_time_breakdown is not None:
            phase_times["Mooring and Foundations"] = sum(
                mooring_component_time_breakdown.values()
            )
        ...
        if device_time_class_breakdown is not None:
            ...
            for class_name in time_class_names:
                time_classes[class_name] += device_time_class_breakdown[
                    class_name
                ]
        ...
        if electrical_time_class_breakdown is not None:
            ...
            for class_name in time_class_names:
                time_classes[class_name] += electrical_time_class_breakdown[
                    class_name
                ]
        ...
        if mooring_time_class_breakdown is not None:
            ...
            for class_name in time_class_names:
                time_classes[class_name] += mooring_time_class_breakdown[
                    class_name
                ]
        ...
        if not record_classes:
            ...
            # Assert against module values
            assert np.isclose(
                time_classes["Waiting"],
                installation_output["TIME"]["Total Waiting Time [h]"],
            )

            assert np.isclose(
                time_classes["Preparation"],
                installation_output["TIME"]["Total Preparation Time [h]"],
            )

            assert np.isclose(
                time_classes["Transit"],
                installation_output["TIME"]["Total Sea Transit Time [h]"],
            )

            assert np.isclose(
                time_classes["Operations"],
                installation_output["TIME"]["Total Sea Operation Time [h]"],
            )
        ...
        if phase_times is not None and time_classes is not None:
            ...
            assert np.isclose(
                total_times,
                installation_output["TIME"]["Total Installation Time [h]"],
            )
    ...
    def get_input_dict(
        ...
        name_map = {
            ...
            "Deployment Equipment Footprint": "Deployment eq. footprint [m^2]",
        ...
        name_map = {
            ...
            "Min Cable Bending Radius": "Min cable bending radius [m]",
            "Additional Equipment Footprint": "AE footprint [m^2]",
            "Additional Equipment Mass": "AE weight [t]",
        ...
        equipment = {
            ...
            "cable burial": EquipmentType("cable burial", cable_burial_df),
            "excavating": EquipmentType("excavating", excavating_df),
            "mattress": EquipmentType("mattress", mattresses_df),
            "rock filter bags": EquipmentType("rock_filter_bags", rock_bags_df),
            "split pipe": EquipmentType("split pipe", split_pipes_df),
            "hammer": EquipmentType("hammer", hammer_df),
            "drilling rigs": EquipmentType("drilling rigs", drilling_rigs_df),
            "vibro driver": EquipmentType("vibro driver", vibro_driver_df),
        }
        ...
        name_map = {
            ...
            "Max Gantry Crane Lift Capacity": "Max gantry crane lift capacity [t]",
            "Max Tower Crane Lift Capacity": "Max tower crane lift capacity [t]",
        ...
        ports_df = ports_df.replace({"Jacking capability [yes/no]": yes_no_map})
        ...
        name_map = {
            ...
            "Turntable Inner Diameter": "Turntable inner diameter [m]",
            "Cable Splice Capabilities": "Cable splice [yes/no]",
            "Dynamic Positioning Capabilities": "DP [-]",
            "Max Jack-Up Water Depth": "JackUp max water depth [m]",
            ...
            "Anchor Handling Winch Rated Pull": "AH winch rated pull [t]",
            ...
            "Max Jacking Current Velocity": "OLC: Jacking maxCs [m/s]",
            "Max Jacking Wind Velocity": "OLC: Jacking maxWs [m/s]",
            "Max Transit Hs": "OLC: Transit maxHs [m]",
            "Max Transit Tp": "OLC: Transit maxTp [s]",
            "Max Transit Current Velocity": "OLC: Transit maxCs [m/s]",
            "Max Transit Wind Velocity": "OLC: Transit maxWs [m/s]",
        ...
        site_df.columns = [
            " ".join(col).strip() for col in site_df.columns.values
        ]
        ...
        site_df["layer 1 type"] = site_df["layer 1 type"].map(soil_map)
        ...
        if data.export is not None:
            ...
            export_df.columns = [
                " ".join(col).strip() for col in export_df.columns.values
            ]
            ...
            export_df["layer 1 type"] = export_df["layer 1 type"].map(soil_map)
            ...
            whole_area = whole_area.drop_duplicates(
                subset=["x coord [m]", "y coord [m]"]
            )
        ...
        name_map = {"Prime Mover": "A", "PTO": "B", "Support Structure": "D"}
        ...
        if data.transportation_method == "Tow" and data.bollard_pull is None:
            errStr = (
                "If device is towed fo deployment, a bollard pull value "
                "must be given."
            )
            raise ValueError(errStr)
        ...
        technique_map = {
            "Drilling rig": "Drilling rig [m/h]",
            "Hammer": "Hammer [m/h]",
            "Vibro driver": "Vibro driver [m/h]",
            "ROV with suction pump": "ROV with suction pump [m/h]",
        ...
        port_safety_factor_df = port_safety_factor_df.apply(
        ...
        if np.isnan(port_safety_factor_df["Safety factor (in %) [-]"]).any():
            logMsg = (
                "Detected missing safety factors for ports. Setting to " "1"
            )
        ...
        if np.isnan(vessel_safety_factor_df["Safety factor (in %) [-]"]).any():
            logMsg = (
                "Detected missing safety factors for vessels. Setting " "to 1"
            )
            module_logger.warning(logMsg)
        ...
        column_map = {
            "Parameter": "Equipment parameter and unit [-]",
            "Safety Factor": "Safety factor (in %) [-]",
        }
        ...
        equipment_safety_factor_df = pd.concat(mapped_tables, ignore_index=True)
        ...
        # Fix nans
        if np.isnan(
            equipment_safety_factor_df["Safety factor (in %) [-]"]
        ).any():
            logMsg = (
                "Detected missing safety factors for equipment. Setting " "to 1"
            )
            module_logger.warning(logMsg)
        ...
        # Installation order
        device_order_dict = {
            "Default order": {12: 0, 13: 1},
            "id": {12: "S_structure", 13: "Device"},
            "logistic phases group": {
                12: "Installation of devices",
                13: "Installation of devices",
            },
        }
        ...
        if electrical_network is not None:
            ...
            electrical_order_dict = {
                "Default order": {0: 1, 1: 3, 2: 4, 3: 0, 4: 2, 5: 4},
                "id": {
                    0: "E_export",
                    1: "E_array",
                    2: "E_external",
                ...
                "logistic phases group": {
                    0: "Installation of electrical infrastructure",
                    1: "Installation of electrical infrastructure",
                    2: "Installation of static cable external protection",
                    3: "Installation of electrical infrastructure",
                    4: "Installation of electrical infrastructure",
                    5: "Installation of electrical infrastructure",
                },
            }
        ...
        if moorings_foundations_network is not None:
            ...
            mf_order_dict = {
                "Default order": {6: 0, 7: 0, 8: 0, 9: 0, 10: 0, 11: 0},
                "id": {
                    6: "Driven",
                    7: "Gravity",
                    8: "M_pile",
                ...
                "logistic phases group": {
                    6: "Installation of moorings & foundations",
                    7: "Installation of moorings & foundations",
                    8: "Installation of moorings & foundations",
                    9: "Installation of moorings & foundations",
                    10: "Installation of moorings & foundations",
                    11: "Installation of moorings & foundations",
                },
            }
        ...
        device, x, y = zip(
            *[
                (key.lower(), item.x, item.y)
                for key, item in array_layout.items()
            ]
        )

        layout_dict = {"Device": device, "UTM X": x, "UTM Y": y}
        ...
        if electrical_network is not None:
            if data.export is None:
                errStr = (
                    "Cable corridor bathymetry must be provided"
            ...
            if missing_titles:
                ...
                errStr = (
                    "Complete electrical network data has not been "
                    "provided. {} with {} {} must be "
                    "satisfied."
                ).format(var_str, title_str, all_missing)
                raise ValueError(errStr)

            collection_point_df = set_collection_points(
                elec_component_data_df,
                elec_network_design,
                elec_hierarchy,
            ...
            name_map = {
                ...
                "Pigtail Cable Mass": "pigtails cable dry mass [kg/m]",
            ...
            static_cable_df = set_cable_cp_references(
                static_cable_df, collection_point_df
            )
            ...
            name_map = {
                ...
                "Upstream Interface Type": "upstream ei type [-]",
                "Downstream Interface Type": "downstream ei type [-]",
                "Upstream Component Type": "upstream termination type [-]",
                "Upstream Component Id": "upstream termination id [-]",
                "Downstream Component Type": "downstream termination type [-]",
                "Downstream Component Id": "downstream termination id [-]",
                "Mass": "dry mass [kg/m]",
            ...
            if "floating" in system_type.lower():
                if data.elec_db_dynamic_cable is None:
                    errStr = (
                        "The dynamic electrical cable database must be "
                ...
                if data.umbilical_terminations is None:
                    errStr = (
                        "The umbilical seabed connection points must be "
                        "provided for electrical network installation "
                        "of floating devices"
                    )
                    raise ValueError(errStr)

                dynamic_cable_df = set_cables(
                    elec_component_data_df,
                    elec_network_design,
                    elec_hierarchy,
                ...
                umbilical_ends = {
                    name.lower(): val
                    for name, val in data.umbilical_terminations.items()
                }
                ...
                dynamic_cable_df = pd.merge(
                    dynamic_cable_df, terminations, on=["Marker"]
                )

                dynamic_cable_df = set_cable_cp_references(
                    dynamic_cable_df, collection_point_df
                )
                ...
                name_map = {
                    ...
                    "MBL": "MBL [N]",
                    "Total Mass": "total dry mass [kg]",
                    "Upstream UTM X": "upstream termination x coord [m]",
                    "Upstream UTM Y": "upstream termination y coord [m]",
                    "Downstream UTM X": "downstream termination x coord [m]",
                    "Downstream UTM Y": "downstream termination y coord [m]",
                }
                ...
                dynamic_cable_df.loc[:, "buoyancy number [-]"] = pd.Series(
                    buoyancy_number
                )

                dynamic_cable_df.loc[:, "buoyancy length [m]"] = pd.Series(
                    buoyancy_length
                )

                dynamic_cable_df.loc[:, "buoyancy weigth [kg]"] = pd.Series(
                    buoyancy_weight
                )

                dynamic_cable_df.loc[:, "buoyancy diameter [mm]"] = pd.Series(
                    buoyancy_diameter
                )

                dynamic_cable_df.loc[:, "dry mass [kg/m]"] = pd.Series(
                    cable_weight
                )
            ...
            cable_route_df["bathymetry [m]"] = -cable_route_df["bathymetry [m]"]
            ...
            cable_route_df["soil type [-]"] = cable_route_df[
                "soil type [-]"
            ].map(soil_map)

            connector_db = elec_dry_mate_df.append(elec_wet_mate_df, sort=False)

            connectors_df = set_connectors(elec_component_data_df, connector_db)

            connectors_df.index.name = "id [-]"

            name_map = {
                "Demating Force": "demating force [N]",
                "Height": "height [m]",
                "Length": "lenght [m]",
                "Mass": "dry mass [kg]",
            ...
            external_protection_df = pd.DataFrame(
                external_protection, dtype=object
            )
        ...
        arg_dict = {
            ...
            "equipment_safety_factor_df": equipment_safety_factor_df,
    ...
    def _init_phase_dicts(self):
        cost_dict = {"Component": [], "Equipment": [], "Port": [], "Vessel": []}
        ...
        date_dict = {"Component": [], "Start": [], "Depart": [], "End": []}
