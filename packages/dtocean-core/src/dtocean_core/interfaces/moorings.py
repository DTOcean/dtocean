...
from dtocean_moorings.main import Main, Variables
...
class MooringsInterface(ModuleInterface):
    ...
    def declare_inputs(cls):
        ...
        input_list = [
            ...
            MaskVariable(
                "device.depth_variation_permitted",
                "device.system_type",
            ...
            MaskVariable(
                "project.mooring_ALS_safety_factor",
                "device.system_type",
                ["Tidal Floating", "Wave Floating"],
            ),
            MaskVariable(
                "project.mooring_ULS_safety_factor",
                "device.system_type",
                ["Tidal Floating", "Wave Floating"],
            ),
    ...
    def declare_id_map(self):
        ...
        id_map = {
            ...
            "currentdragcoefrect": "constants.rectangular_current_drag",
            ...
            "hcfdrsoil": "constants.soil_drained_holding_capacity_factor",
            ...
            "pilefricresnoncal": "constants.pile_skin_friction_end_bearing_capacity",
            ...
            "subgradereaccoef": "constants.soil_cohesionless_reaction_coefficient",
            ...
            "wakeampfactorcyl": "constants.cylinder_wake_amplificiation",
            ...
            "waveinertiacoefrect": "constants.rectangular_wave_inertia",
            ...
            "economics_data": "project.moorings_foundations_economics_data",
    ...
    def connect(self, debug_entry=False, export_data=False):
        ...
        soil_table = np.dstack(
            [xv.flatten(), yv.flatten(), sv.flatten(), infv.flatten()]
        )[0]
        ...
        # Soil properties
        name_map = {
            "Drained Soil Friction Angle": "dsfang",
        ...
        if "Tidal" in self.data.system_type:
            ...
            thrustcurv = (
                self.data.turbine_performance["Coefficient of Thrust"]
                .reset_index()
                .values
            )
        ...
        # Device layout
        sysorig = {
            key.lower(): np.append(position, 0.0).tolist()
        ...
        if self.data.substparams is None:
            ...
            for sub_id in substation_props.index:
                local_orig = list(self.data.substation_layout[sub_id].coords[0])
                if len(local_orig) == 2:
                    local_orig.append(0.0)

                suborig[sub_id] = str(local_orig)
                subcog[sub_id] = str(self.data.substation_cog[sub_id].tolist())

                substloc[sub_id] = str(
                    self.data.substation_foundations[sub_id].tolist()
                )

            substation_props = pd.concat(
                [substation_props, suborig, subcog, substloc], axis=1
            )

        ## COMPONENTS
        compdict = self.get_all_components(
        ...
        if "floating" in self.data.system_type.lower():
            ...
            if self.data.preumb in compdict:
                ...
                errStr = (
                    "Selected umbilical component '{}' not found in "
                    "component dictionary"
                ).format(self._variables.preumb)
        ...
        if (
            main.sysfoundinsttab["type [-]"]
            .isin(["Foundation solution not found"])
            .any()
        ):
        ...
        raw_network = {"topology": main.syshier, "nodes": main.sysrambom}
        ...
        foundations_data[change_cols] = foundations_data[change_cols].apply(
        ...
        self.data.foundations_data = foundations_data.rename(columns=name_map)
        ...
        foundations_layers["Soil Type"] = foundations_layers["Soil Type"].map(
            inv_soil_map
        )
        ...
        for dev_id, env_dict in env_data.iteritems():
            ...
            footprint_list.append(env_dict["Configuration footprint area [m2]"])
            volume_list.append(env_dict["Configuration volume [m3]"][0])

        raw_env = {
            "System Identifier": dev_list,
        ...
        if "floating" in self.data.system_type.lower():
            ...
            moorings_raw = {"Line Identifier": line_list, "Marker": marker_list}
            moorings_data = pd.DataFrame(moorings_raw)

            line_data = moorings_df.drop(["devices [-]", "marker [-]"], axis=1)
    ...
    def get_all_components(
        ...
        pile_dict = get_component_dict(
            "pile", foundations_pile, check_keys=compdict.keys()
        )
        compdict.update(pile_dict)

        # Moorings
        if "floating" in system_type.lower():
            chain_dict = get_component_dict(
                "chain", moorings_chain, check_keys=compdict.keys()
            )
            compdict.update(chain_dict)

            forerunner_dict = get_component_dict(
                "forerunner assembly",
                moorings_forerunner,
            ...
            shackle_dict = get_component_dict(
                "shackle", moorings_shackle, check_keys=compdict.keys()
            )
            compdict.update(shackle_dict)

            swivel_dict = get_component_dict(
                "swivel", moorings_swivel, check_keys=compdict.keys()
            )
            compdict.update(swivel_dict)

            umbilical_dict = get_component_dict(
                "cable", moorings_umbilical, check_keys=compdict.keys()
            )
