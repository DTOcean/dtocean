...
from dtocean_environment.main import (
    ElectricalStage,
    HydroStage,
    InstallationStage,
    MooringStage,
...
class EnvironmentalInterface(ThemeInterface):
    ...
    def declare_id_map(self):
        ...
        id_map = {
            ...
            "hydro_collision_risk_weight": "project.hydro_collision_risk_weight",
            ...
            "elec_resting_place_weight": "project.elec_resting_place_weight",
    ...
    def connect(self, debug_entry=False, export_data=False):
        ...
        protected_table_df2 = pd.DataFrame(
            protected_dict.items(), columns=name_protected
        )
        ...
        mapping_index = {
            "Observation of Hard substrate benthic habitat": "Hard substrate benthic habitat",
        ...
        if self.data.receptor_species is None:
            values = [
                ["Hard substrate benthic habitat"] + null_months,
            ...
            receptors_species_df.rename(columns=mapping_columns, inplace=True)
            receptors_species_df.replace(
                ["true", "false", "unknown"], [True, False, None], inplace=True
            )
        ...
        if self.data.hydro_collision_risk_weight is None:
            ...
            translate_hydro_collision_risk_dict = {
                "Open Water/ Devices In Parallel": "open water/ devices in parallel",
        ...
        if self.data.hydro_underwater_noise_risk_weight is None:
            ...
            translate_hydro_underwater_noise_risk_dict = {
                "Noise Device 0 - 90 dB re 1muPa": "noise device 0 - 90 dB re 1muPa",
        ...
        if self.data.hydro_resting_place_weight is None:
            ...
            translate_hydro_resting_place_weight_dict = {
                "No Dangerous Part Of Devices": "No dangerous part of devices",
                "Blades": "blades",
        ...
        if self.data.layout is None:
            ...
            device_coord_three_coords = [
                list(item.coords)[0] for item in self.data.layout.values()
            ]
        ...
        if None not in (
            ...
            device_emerged_surface = (
                self.data.device_width * self.data.device_length
            )

        elif device_wet_height is None and None not in (
            self.data.device_width,
            self.data.device_length,
        ):
        ...
        HydroStage_assessment = HydroStage(
            protected_table_df, receptor_table_df, hydro_weighting_dict
        )
        ...
        (
            ...
            hydro_season,
            hydro_global_eis,
        ) = HydroStage_assessment(hydro_input_dict)
        ...
        if self.data.elec_elec_field_weight is None:
            ...
            translate_elec_elec_field_dict = {
                "Cable Buried": "cable buried",
                "Cable Not Buried": "cable not buried",
            }
        ...
        if self.data.elec_underwater_noise_risk_weight is None:
            ...
            translate_elec_underwater_noise_risk_dict = {
                "Noise Electrical Components 0 - 90 dB re 1muPa": "noise electrical components 0 dB re 1muPa",
        ...
        if self.data.substation_layout is None:
            ...
            substation_coord_three_coords = [
                list(item.coords)[0]
                for item in self.data.substation_layout.values()
            ]

            substation_coord_x = [
                item[0] for item in substation_coord_three_coords
            ]
            substation_coord_y = [
                item[1] for item in substation_coord_three_coords
            ]
            ...
            size_of_device = max(
                self.data.substation_props["Length"].max(),
            ...
            substation_surface_area_covered = substation_props_df[
                "Cross Area"
            ].sum()
            substation_emerged_surface = substation_props_df["Cross Area"].max()

            substation_props_df["Underwater Area"] = (
                2
                * (
                    substation_props_df["Length"] * substation_props_df["Width"]
                    + substation_props_df["Height"]
                    * substation_props_df["Width"]
                    + substation_props_df["Length"]
                    * substation_props_df["Height"]
                )
        ...
        ElecStage_assessment = ElectricalStage(
            protected_table_df, receptor_table_df, elec_weighting_dict
        )
        ...
        (
            ...
            elec_season,
            elec_global_eis,
        ) = ElecStage_assessment(elec_input_dict)
        ...
        if any(x is not None for x in elec_season.values):
            ...
            self.data.elec_season.columns = [
                x.title() for x in self.data.elec_season.columns
            ]
        ...
        if self.data.moorings_dimensions is not None:
            ...
            moor_size["diameter"] = 2 * np.sqrt(
                moor_size["Footprint Area"] / np.pi
            )
            moor_size["surface"] = (
        ...
        MoorStage_assessment = MooringStage(
            protected_table_df, receptor_table_df, moor_weighting_dict
        )
        #
        (
            moor_confidence,
            ...
            moor_season,
            moor_global_eis,
        ) = MoorStage_assessment(moor_input_dict)
        ...
        if any(x is not None for x in moor_season.values):
            ...
            self.data.moor_season.columns = [
                x.title() for x in self.data.moor_season.columns
            ]
        ...
        if (
            self.data.number_vessels is not None
            and self.data.average_size_vessels_inst is not None
        ):
            surface_covered = (
                self.data.number_vessels
                * np.pi
                * (0.5 * self.data.average_size_vessels_inst) ** 2
            )
        else:
        ...
        InstallStage_assessment = InstallationStage(
            protected_table_df, receptor_table_df, install_weighting_dict
        )
        #
        (
            install_confidence,
            ...
            install_season,
            install_global_eis,
        ) = InstallStage_assessment(install_input_dict)
        ...
        if any(x is not None for x in install_season.values):
            ...
            self.data.install_season.columns = [
                x.title() for x in self.data.install_season.columns
            ]
        ...
        if self.data.operat_underwater_noise_risk_weight is None:
            ...
            operat_underwater_noise_risk_weight_map = (
                translate_operat_underwater_noise_risk_dict[
        ...
        if self.data.operat_chemical_pollution_weight is None:
            ...
            operat_chemical_pollution_weight_map = (
                translate_operat_chemical_pollution_dict[
        ...
        if (
            self.data.operat_number_vessels is not None
            and self.data.operat_average_size_vessels is not None
        ):
            operat_surface_covered = (
                self.data.operat_number_vessels
                * np.pi
                * (0.5 * self.data.operat_average_size_vessels)
                ^ 2
            )
        else:
        ...
        OperatStage_assessment = OperationMaintenanceStage(
            protected_table_df, receptor_table_df, operat_weighting_dict
        )
        #
        (
            operat_confidence,
            ...
            operat_season,
            operat_global_eis,
        ) = OperatStage_assessment(operat_input_dict)
        ...
        if any(x is not None for x in operat_season.values):
            ...
            self.data.operat_season.columns = [
                x.title() for x in self.data.operat_season.columns
            ]
        ...
        for key in keys:
            impacts = [
                impact[key] for impact in global_list if impact is not None
            ]
