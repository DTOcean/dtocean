...
from mdo_engine.boundary.interface import QueryInterface, MaskVariable
...
class SystemTypeInterface(ProjectInterface):
    ...
    def declare_id_map(self):
        ...
        id_map = {"not_used": "device.system_type",
...
class OptionsInterface(ProjectInterface):
    ...
    def declare_outputs(cls):
        ...
        output_list = ["device.available_names",
    ...
    def declare_id_map(self):
        ...
        id_map = {"sys_type": "device.system_type",
...
class SiteBoundaryInterface(ProjectInterface):
    ...
    def declare_inputs(cls):
        ...
        input_list  =  ["hidden.available_sites",
                        "hidden.corridor_boundaries",
                        "hidden.lease_boundaries",
                        "hidden.landing_points",
                        "site.selected_name",
    ...
    def declare_outputs(cls):
        ...
        output_list = ["site.corridor_boundary",
                       "site.lease_boundary",
                       "site.projection",
    ...
    def declare_id_map(self):
        ...
        id_map = {"all_sites": "hidden.available_sites",
    ...
    def connect(self):
        ...
        if np.sum(site_matches) == 0:
            ...
            errStr = ("No site with name '{}' found. Available names "
                      "are {}").format(self.data.selected_site,
                                       names_str)
...
class FilterInterface(ProjectInterface, QueryInterface):
    ...
    def declare_outputs(cls):
        ...
        outputs = ["hidden.site_filtered",
                   "hidden.device_filtered"]
    ...
    def connect(self):
        ...
        if self.data.selected_system is not None:
            ...
            systems_df = self.data.all_systems
            system_matches = systems_df[
                                    "description"] == self.data.selected_system
            system_ids = systems_df[system_matches]["id"]
            
            if len(system_ids) == 0:
                
                names_str = ", ".join(systems_df["description"])
                errStr = ("No system with name '{}' found. Available names "
                          "are {}").format(self.data.selected_system,
                                           names_str)
                raise ValueError(errStr)
            
            if len(system_ids) > 1:
                
                errStr = ("More than one system with name '{}' found. "
                          "Database is corrupt.").format(
