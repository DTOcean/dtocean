...
#    Copyright (C) 2018-2024 Mathew Topper
#
...
class CombinedBathyPlot(PlotInterface):
    ...
    def declare_id_map(cls):
        ...
        id_map = {
            "deployment_strata": "bathymetry.layers",
    ...
    def connect(self):
        deployment_bathy = self.data.deployment_strata["depth"].sel(
            layer="layer 1"
        )
        cable_corridor_bathy = self.data.cable_corridor_strata["depth"].sel(
            layer="layer 1"
        )
...
class CombinedSedimentPlot(PlotInterface):
    ...
    def connect(self):
        ...
        deployment_sediment = (
            self.data.deployment_strata["sediment"].sel(layer="layer 1").values
        )
        cable_corridor_sediment = (
            self.data.cable_corridor_strata["sediment"]
            .sel(layer="layer 1")
            .values
        )
        ...
        cs = plt.contourf(
            x, y, new_deployment_sediment.T, colors=colors, levels=levels
        )
        ...
        plt.contourf(
            x, y, new_cable_corridor_sediment.T, colors=colors, levels=levels
        )
