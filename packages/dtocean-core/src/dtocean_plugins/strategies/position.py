...
class OptimiserThread(threading.Thread):
    def __init__(self, optimiser, config, log_interval=100, wait_interval=1):
    ...
    def _set_stopped(self):
        ...
        try:
            ...
            log_msg = ("Exit hook threw {}: " "{}").format(
    ...
    def run(self):
        ...
        while not self._optimiser.stop:
            if (
                self._continue_event.is_set() != continue_event_state
                and not self._continue_event.is_set()
            ):
            ...
            if (
                self._continue_event.is_set() != continue_event_state
                and self._continue_event.is_set()
            ):
...
class AdvancedPosition(Strategy):
    ...
    def get_config_status(cls, config):
        ...
        required_filled = [
            bool(config[x]) if x in config else False for x in required_keys
        ]
    ...
    def _pre_execute(self, core, project):
        ...
        config_copy = deepcopy(self._config)
        optimiser = PositionOptimiser(
            core=core, config_fname=self.get_config_fname()
        )
    ...
    def _prepare_project(self, core, project):
        ...
        status_str, status_code = self.get_project_status(
            core, project, self._config
        )
    ...
    def get_favorite_result(cls, config):
        ...
        result_dict = _read_yaml(xfavorite_path, read_params, extract_vars)
    ...
    def import_simulation_file(
        self, core, project, yaml_file_path, sim_title=None
    ):
    ...
    def allow_run(cls, core, project, config):
        _, project_status_code = AdvancedPosition.get_project_status(
            core, project, config
        )

        if project_status_code == 0:
            return False
    ...
    def get_optimiser_status(cls, core, config):
        ...
        optimiser = PositionOptimiser(
            core=core, config_fname=cls.get_config_fname()
        )
    ...
    def get_project_status(cls, core, project, config):
        ...
        if not set(required_modules) <= set(active_modules):
            ...
            for missing in set(required_modules) - set(active_modules):
                status_str = (
                    "Project does not contain the {} " "module"
                ).format(missing)
        ...
        if "Default" not in project.get_simulation_titles():
            status_str = (
                "The position optimiser requires a simulation "
                'with title "Default"'
            )
            return status_str, 0

        if "objective" not in config or config["objective"] is None:
        ...
        if objective not in sim.get_output_ids():
            status_str = (
                "Objective {} is not an output of the default " "simulation"
            ).format(objective)
            return status_str, 0
...
def _run_favorite(
    optimiser, raise_exc=False, save_prj=False, disable_iterate_logging=True
):
    ...
    params = [
        x.item() if type(x).__module__ == np.__name__ else x for x in params
    ]

    if nh is not None:
        params += [nh.last_n_evals]
    ...
    try:
        iterate(core, project, positioner, *params)
...
def _post_process(config, log_interval=100):
    ...
    for i, yaml_file_path in enumerate(yaml_file_paths):
        if (i + 1) % log_interval == 0:
            msg_str = "Processed {} of {} simulations".format(i + 1, n_sims)
            module_logger.info(msg_str)
        ...
        yaml_dict = _read_yaml(yaml_file_path, read_params, extract_vars)
    ...
    for key, item in pickle_dict.copy().items():
        if not item:
            pickle_dict.pop(key)
...
def _read_yaml(yaml_file_path, read_params, extract_vars):
    ...
    if set(param_values).isdisjoint(set(mapped_params)) and set(
        data_values
    ).isdisjoint(set(extract_vars)):
        return {}
    ...
    for var_name in extract_vars:
        if var_name not in data_values:
            continue
