...
class UnitSensitivity(Strategy):
    ...
    def execute(self, core, project):
        ...
        if (
            ...
            or self._config["module_name"] is None
            or self._config["var_name"] is None
            or self._config["var_values"] is None
        ):
            errStr = (
                "Some configuration values are None. Have you called "
        ...
        # Pick up the branch
        if module_name not in self._module_menu.get_available(core, project):
            errStr = "Module {} does not exist".format(module_name)
            raise ValueError(errStr)

        if module_name not in self._module_menu.get_active(core, project):
