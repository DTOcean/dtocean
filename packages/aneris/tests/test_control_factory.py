...
from aneris.boundary.interface import AutoInterface, RawInterface, FileInterface
from aneris.control.factory import InterfaceFactory

from aneris.entity.data import DataCatalog, MetaData
...
def test_make_auto_test():
    ...
    meta = AutoMeta({"identifier": "Not:Real:Variable",
                     "structure": None,
                     "auto": 2})
    
    test_simple = AutoSimple()

    TestCls = make_auto_interface(meta,
                                  test_simple)
...
def test_make_auto_raw():
    ...
    assert TestCls.declare_inputs() == None
    assert TestCls.declare_optional() == None
    assert TestCls.declare_outputs() == ["Not:Real:Variable"]
    assert TestCls.declare_id_map() == {"result": "Not:Real:Variable"}
...
def test_build_auto():
    ...
    assert catalog._metadata_variable_map[
                                "my:auto:variable"].structure == "AutoSimple"
    ...
    TestCls = make_auto_interface(metadata,
                                  data_obj)                                  
